...
SetStatusLabel(not_null<Main::Session *> session) {
  const auto self = session->user();
  return session->changes().peerFlagsValue(
             self, Data::PeerUpdate::Flag::EmojiStatus) |
         rpl::map([=] { return !!self->emojiStatusId(); }) |
         rpl::distinct_until_changed() | rpl::map([](bool has) {
           const auto makeLink = [](const QString &text) {
             return tr::link(text);
           };
           return (has ? tr::lng_menu_change_status
...
public:
  ...
  [[nodiscard]] int rightSkip() const { return _rightSkip.current(); }
...
MainMenu::ToggleAccountsButton::ToggleAccountsButton(
    QWidget *parent, not_null<Main::Account *> current)
    : AbstractButton(parent), _current(current) {
  rpl::single(rpl::empty) | rpl::then(Core::App().unreadBadgeChanges()) |
      rpl::on_next(
          [=] {
  ...
  settings.mainMenuAccountsShownValue() |
      rpl::filter([=](bool value) { return (_toggled != value); }) |
      rpl::on_next(
          [=](bool value) {
            _toggled = value;
            _toggledAnimation.start([=] { update(); }, _toggled ? 0. : 1.,
                                    _toggled ? 1. : 0., st::slideWrapDuration);
            validateUnreadBadge();
          },
          lifetime());
...
void MainMenu::ToggleAccountsButton::paintEvent(QPaintEvent *e) {
  ...
  const auto path = Ui::ToggleUpDownArrowPath(
      ...
      0. + height() - st::mainMenuTogglePosition.y(), st::mainMenuToggleSize,
...
void MainMenu::ToggleAccountsButton::paintUnreadBadge(Painter &p) {
  ...
  const auto right =
      width() - st::mainMenuTogglePosition.x() - st::mainMenuToggleSize * 3;
  const auto top =
      height() - st::mainMenuTogglePosition.y() - st::mainMenuBadgeSize / 2;
  p.setOpacity(progress);
  Ui::PaintUnreadBadge(p, _unreadBadge, right, top, st);
}

void MainMenu::ToggleAccountsButton::validateUnreadBadge() {
  const auto base = st::mainMenuTogglePosition.x() + 2 * st::mainMenuToggleSize;
  if (_toggled) {
    _rightSkip = base;
    return;
...
QString MainMenu::ToggleAccountsButton::computeUnreadBadge() const {
  ...
  return state.allMuted      ? QString()
         : (state.count > 0) ? Lang::FormatCountToShort(state.count).string
                             : QString();
}

MainMenu::ResetScaleButton::ResetScaleButton(QWidget *parent)
    : AbstractButton(parent) {
  const auto margin = st::mainMenuCloudButton.height - st::mainMenuCloudSize;
  const auto textWidth = st::mainMenuResetScaleFont->width(kText);
  const auto innerWidth =
      st::mainMenuResetScaleLeft + textWidth + st::mainMenuResetScaleRight;
...
void MainMenu::ResetScaleButton::paintEvent(QPaintEvent *e) {
  ...
  st::settingsIconInterfaceScale.paint(
      p, left + st::mainMenuResetScaleIconLeft,
      top + ((innerHeight - st::settingsIconInterfaceScale.height()) / 2),
      width(), st::mainMenuCloudFg->c);

  p.setFont(st::mainMenuResetScaleFont);
  p.setPen(st::mainMenuCloudFg);
  p.drawText(left + st::mainMenuResetScaleLeft,
             top + st::mainMenuResetScaleTop +
                 st::mainMenuResetScaleFont->ascent,
             kText);
}

MainMenu::MainMenu(QWidget *parent, not_null<SessionController *> controller)
    : LayerWidget(parent), _controller(controller),
      _userpicButton(this, _controller->session().user(), st::mainMenuUserpic),
      ...
      _accounts(_inner->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
          _inner.get(), object_ptr<Ui::VerticalLayout>(_inner.get())))),
      _shadow(_inner->add(object_ptr<Ui::SlideWrap<Ui::PlainShadow>>(
          _inner.get(), object_ptr<Ui::PlainShadow>(_inner.get())))),
      _menu(_inner->add(object_ptr<Ui::VerticalLayout>(_inner.get()),
                        {0, st::mainMenuSkip, 0, 0})),
      _footer(_inner->add(object_ptr<Ui::RpWidget>(_inner.get()))),
      _telegram(Ui::CreateChild<Ui::FlatLabel>(_footer.get(),
                                               st::mainMenuTelegramLabel)),
  ...
  _footer->heightValue() |
      rpl::on_next(
          [=] {
            _telegram->moveToLeft(st::mainMenuFooterLeft,
                                  _footer->height() -
                                      st::mainMenuTelegramBottom -
                                      _telegram->height());
            _version->moveToLeft(st::mainMenuFooterLeft,
                                 _footer->height() - st::mainMenuVersionBottom -
                                     _version->height());
          },
          _footer->lifetime());

  rpl::combine(heightValue(), _inner->heightValue()) |
      rpl::on_next([=] { updateInnerControlsGeometry(); }, _inner->lifetime());
  ...
  _version->setLink(
      1, std::make_shared<UrlClickHandler>(Core::App().changelogLink()));
  _version->setLink(2, std::make_shared<LambdaClickHandler>(
                           [=] { controller->show(Box(AboutBox)); }));

  rpl::combine(_toggleAccounts->rightSkipValue(),
               rpl::single(rpl::empty) | rpl::then(_badge->updated())) |
      rpl::on_next([=] { moveBadge(); }, lifetime());
  ...
  if (CanCheckSpecialEvent() && CheckSpecialEvent()) {
    ...
    const auto rebuild = [=] {
      ...
      _showFinished.value() |
          rpl::on_next([=](bool shown) { snow->setPaused(!shown); },
                       snowRaw->lifetime());
      snowRaw->paintRequest() | rpl::on_next(
                                    [=](const QRect &r) {
...
void MainMenu::setupArchive() {
    ...
    if (const auto f = folder()) {
      if (modifiers & Qt::ControlModifier) {
        controller->showInNewWindow(Window::SeparateId(
            Window::SeparateType::Archive, &controller->session()));
      } else {
  ...
  const auto checkArchive = [=] {
    const auto f = folder();
    return f && (!f->chatsList()->empty() || f->storiesCount() > 0) &&
           controller->session().settings().archiveInMainMenu();
  };

  const auto wrap = _menu->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
      _menu, object_ptr<Ui::VerticalLayout>(_menu)));
  const auto inner = wrap->entity();
  wrap->toggle(checkArchive(), anim::type::instant);

  const auto button =
      AddButtonWithIcon(inner, tr::lng_archived_name(), st::mainMenuButton,
                        {&st::menuIconArchiveOpen});
  inner->add(object_ptr<Ui::PlainShadow>(inner),
             {0, st::mainMenuSkip, 0, st::mainMenuSkip});
  button->setAcceptBoth(true);
  button->clicks() |
      rpl::on_next(
          [=](Qt::MouseButton which) {
  ...
  auto folderValue =
      now ? (rpl::single(now) | rpl::type_erased)
  ...
  Badge::AddUnread(
      ...
      rpl::single(rpl::empty) |
          ...
          rpl::map([=] {
            ...
            const auto state =
                loaded ? loaded->chatListBadgesState() : Dialogs::BadgesState();
            return Badge::UnreadBadge{state.unreadCounter, true};
          }));

  rpl::merge(controller->session().data().chatsListChanges() |
                 rpl::filter([](Data::Folder *folder) {
                   return folder && (folder->id() == Data::Folder::kId);
                 }) |
                 rpl::to_empty,
...
void MainMenu::setupAccounts() {
  ...
  std::move(events.closeRequests) |
      rpl::on_next([=] { closeLayer(); }, inner->lifetime());
...
void MainMenu::setupSetEmojiStatus() {
  _setEmojiStatus->overrideLinkClickHandler([=] { chooseEmojiStatus(); });
}
...
void MainMenu::setupMenu() {
  ...
  const auto controller = _controller;
  const auto addAction = [&](rpl::producer<QString> text,
                             IconDescriptor &&descriptor) {
    return AddButtonWithIcon(_menu, std::move(text), st::mainMenuButton,
                             std::move(descriptor));
  };
  if (!_controller->session().supportMode()) {
    _menu
        ->add(CreateButtonWithIcon(_menu, tr::lng_menu_my_profile(),
                                   st::mainMenuButton, {&st::menuIconProfile}))
        ->setClickedCallback([=] {
    ...
    _menu->add(object_ptr<Ui::PlainShadow>(_menu),
               {0, st::mainMenuSkip, 0, st::mainMenuSkip});

    AddMyChannelsBox(
        addAction(tr::lng_create_group_title(), {&st::menuIconGroups}),
        controller, true)
        ->addClickHandler([=](Qt::MouseButton which) {
          if (which == Qt::LeftButton) {
    ...
    AddMyChannelsBox(
        addAction(tr::lng_create_channel_title(), {&st::menuIconChannel}),
        controller, false)
        ->addClickHandler([=](Qt::MouseButton which) {
          if (which == Qt::LeftButton) {
    ...
    addAction(tr::lng_menu_contacts(), {&st::menuIconUserShow})
        ->setClickedCallback(
            [=] { controller->show(PrepareContactsBox(controller)); });
    addAction(tr::lng_menu_calls(), {&st::menuIconPhone})
        ->setClickedCallback([=] { ::Calls::ShowCallsBox(controller); });
    addAction(tr::lng_saved_messages(), {&st::menuIconSavedMessages})
        ->setClickedCallback(
            [=] { controller->showPeerHistory(controller->session().user()); });
  } else {
    addAction(tr::lng_profile_add_contact(), {&st::menuIconProfile})
        ->setClickedCallback([=] { controller->showAddContact(); });
    addAction(rpl::single(u"Fix chats order"_q), {&st::menuIconPin})
            ->toggleOn(rpl::single(
        ...
        rpl::on_next(
            ...
            _menu->lifetime());
    addAction(rpl::single(u"Reload templates"_q), {&st::menuIconRestore})
        ->setClickedCallback(
...
void MainMenu::updateControlsGeometry() {
  ...
  _setEmojiStatus->moveToLeft(st::mainMenuCoverStatusLeft,
                              st::mainMenuCoverStatusTop, width());
  _toggleAccounts->setGeometry(0, st::mainMenuCoverNameTop, width(),
...
void MainMenu::updateInnerControlsGeometry() {
  const auto contentHeight = _accounts->height() + _shadow->height() +
                             st::mainMenuSkip + _menu->height();
  const auto available = height() - st::mainMenuCoverHeight - contentHeight;
  const auto footerHeight = std::max(available, st::mainMenuFooterHeightMin);
...
void MainMenu::drawName(Painter &p) {
  const auto widthText =
      width() - st::mainMenuCoverNameLeft - _toggleAccounts->rightSkip();

  const auto user = _controller->session().user();
  if (_nameVersion < user->nameVersion()) {
    _nameVersion = user->nameVersion();
    _name.setText(st::semiboldTextStyle, user->name(), Ui::NameTextOptions());
    moveBadge();
  }
  p.setFont(st::semiboldFont);
  p.setPen(st::windowBoldFg);
  _name.drawLeftElided(
      p, st::mainMenuCoverNameLeft, st::mainMenuCoverNameTop,
      (widthText - (_badge->widget()
...
void MainMenu::initResetScaleButton() {
  _controller->widget()->screenValue() |
      rpl::map([](not_null<QScreen *> screen) {
        return rpl::single(screen->availableGeometry()) |
               rpl::then(base::qt_signal_producer(
                   screen.get(), &QScreen::availableGeometryChanged));
      }) |
      rpl::flatten_latest() | rpl::map([](QRect available) {
...
OthersUnreadState
OtherAccountsUnreadStateCurrent(not_null<Main::Account *> current) {
...
rpl::producer<OthersUnreadState>
OtherAccountsUnreadState(not_null<Main::Account *> current) {
  return rpl::single(rpl::empty) | rpl::then(Core::App().unreadBadgeChanges()) |
         rpl::map([=] { return OtherAccountsUnreadStateCurrent(current); });
}
