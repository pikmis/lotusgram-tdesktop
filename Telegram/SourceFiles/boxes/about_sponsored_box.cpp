...
constexpr auto kUrl = "https://ads.lotusgram.lol"_cs;
...
void AboutSponsoredBox(not_null<Ui::GenericBox *> box) {
  ...
  const auto addUrl = [&] {
    ...
    const auto button = Ui::CreateChild<RoundButton>(
        row, rpl::single<QString>(kUrl.utf8()), st);
    button->setBrushOverride(Qt::NoBrush);
    button->setPenOverride(QPen(st::historyLinkInFg));
    rpl::combine(row->sizeValue(), button->sizeValue()) |
        rpl::on_next(
            [=](const QSize &rowSize, const QSize &buttonSize) {
              button->moveToLeft((rowSize.width() - buttonSize.width()) / 2,
                                 (rowSize.height() - buttonSize.height()) / 2);
            },
            row->lifetime());
    button->addClickHandler([=] { File::OpenUrl(kUrl.utf8()); });
  };

  const auto &stLabel = st::aboutLabel;
  auto text1 = tr::lng_sponsored_info_description1_linked(
      lt_link,
      rpl::combine(tr::lng_sponsored_info_description1_link(),
