...
public:
  MaybeDisabledEmoji(std::unique_ptr<CustomEmoji> wrapped, Fn<bool()> enabled);
...
MaybeDisabledEmoji::MaybeDisabledEmoji(std::unique_ptr<CustomEmoji> wrapped,
                                       Fn<bool()> enabled)
    : _wrapped(std::move(wrapped)), _enabled(std::move(enabled)) {}

int MaybeDisabledEmoji::width() { return _wrapped->width(); }

QString MaybeDisabledEmoji::entityData() { return _wrapped->entityData(); }
...
void MaybeDisabledEmoji::unload() { _wrapped->unload(); }

bool MaybeDisabledEmoji::ready() { return _wrapped->ready(); }
...
[[nodiscard]] bool
AllowOnlyCustomEmojiMimeDataHook(not_null<const QMimeData *> data,
  ...
  if (action == Ui::InputField::MimeAction::Check) {
    ...
    auto tags =
        TextUtilities::DeserializeTags(data->data(tagsMime), text.size());
    auto checkedTill = 0;
...
[[nodiscard]] std::vector<Data::ReactionId>
CollectAvailableReactions(not_null<Main::Session *> session) {
  const auto &all =
...
[[nodiscard]] bool
RemoveNonCustomEmojiFragment(not_null<QTextDocument *> document,
...
bool RemoveNonCustomEmoji(not_null<QTextDocument *> document,
                          UniqueCustomEmojiContext &context) {
...
[[nodiscard]] TextWithTags
ComposeEmojiList(not_null<Data::Reactions *> reactions,
                 const std::vector<Data::ReactionId> &list) {
  auto result = TextWithTags();
  const auto size = [&] { return int(result.text.size()); };
...
object_ptr<Ui::RpWidget> AddReactionsSelector(not_null<Ui::RpWidget *> parent,
                                              ReactionsSelectorArgs &&args) {
  using namespace ChatHelpers;
  using HistoryView::Reactions::UnifiedFactoryOwner;

  auto result = object_ptr<Ui::InputField>(
      parent, st::manageGroupReactionsField, Ui::InputField::Mode::MultiLine,
      std::move(args.title));
  ...
  state->unifiedFactoryOwner =
      std::make_unique<UnifiedFactoryOwner>(session, normal);
  ...
  context.customEmojiFactory = [=](QStringView data,
    ...
    if (state->unifiedFactoryOwner->lookupReactionId(id).custom()) {
      return std::make_unique<MaybeDisabledEmoji>(
          std::move(result), [=] { return state->allowed.contains(id); });
    }
    using namespace Ui::Text;
    return std::make_unique<FirstFrameEmoji>(std::move(result));
  };
  raw->setCustomTextContext(std::move(context), customEmojiPaused,
                            customEmojiPaused);
  ...
  SetupOnlyCustomEmojiField(
      raw,
      [=](std::vector<DocumentId> ids, bool hardLimitHit) {
      ...
      rpl::on_next(
          [=](bool paid) {
            ...
            if (paid && !ranges::contains(state->reactions, id)) {
              ...
              state->reactions.erase(ranges::remove(state->reactions, id),
                                     end(state->reactions));
              applyFromState();
            }
          },
  ...
  const auto toggle =
      Ui::CreateChild<Ui::IconButton>(parent.get(), st::manageGroupReactions);

  using SelectorState = ReactionsSelectorState;
  std::move(args.stateValue) |
      rpl::on_next(
          [=, all = args.all](SelectorState value) {
            switch (value) {
            case SelectorState::Active:
              state->overlay = nullptr;
              state->focusLifetime.destroy();
              if (raw->empty()) {
                raw->setTextWithTags(ComposeEmojiList(
                    reactions,
                    all ? CollectAvailableReactions(&args.controller->session())
                        : DefaultSelected()));
              }
            ...
            case SelectorState::Disabled:
              ...
              raw->geometryValue() |
                  rpl::on_next(
                      [=](QRect rect) { state->overlay->setGeometry(rect); },
                      state->overlay->lifetime());
              state->overlay->paintRequest() |
                  rpl::on_next(
                      [=](QRect clip) {
                        auto color = st::boxBg->c;
                        color.setAlphaF(0.5);
                        QPainter(state->overlay.get()).fillRect(clip, color);
                      },
                      state->overlay->lifetime());
              [[fallthrough]];
            case SelectorState::Hidden:
              ...
              raw->focusedChanges() | rpl::on_next(
                                          [=](bool focused) {
                                            if (focused) {
                                              raw->parentWidget()->setFocus();
                                            }
  ...
  const auto panel = Ui::CreateChild<TabbedPanel>(
      args.outer.get(), args.controller,
      object_ptr<TabbedSelector>(
          nullptr, args.controller->uiShow(), Window::GifPauseReason::Layer,
          (args.all ? TabbedSelector::Mode::FullReactions
                    : TabbedSelector::Mode::RecentReactions)));
  auto panelList = state->unifiedFactoryOwner->unifiedIdsList();
  panelList.erase(ranges::remove(panelList, paid->selectAnimation->id),
                  end(panelList));
  panel->selector()->provideRecentEmoji(
      ChatHelpers::DocumentListToRecent(panelList));
  panel->setDesiredHeightValues(1., st::emojiPanMinHeight / 2,
  ...
  const auto updateEmojiPanelGeometry = [=] {
    ...
    panel->moveBottomRight(local.y(), local.x() + toggle->width() * 3);
  };
  ...
  for (auto widget = (QWidget *)raw; widget && widget != args.outer;
  ...
  toggle->addClickHandler([=] { panel->toggleAnimated(); });

  raw->geometryValue() |
      rpl::on_next(
          [=](QRect geometry) {
            toggle->move(geometry.x() + geometry.width() - toggle->width(),
...
void AddReactionsText(not_null<Ui::VerticalLayout *> container,
  ...
  container->add(
      object_ptr<Ui::DividerLabel>(container, std::move(ownedInner),
  ...
  const auto label = inner->add(object_ptr<Ui::FlatLabel>(
      ...
      tr::lng_manage_peer_reactions_own(
          lt_link, tr::lng_manage_peer_reactions_own_link(tr::link),
          tr::marked),
      st::boxDividerLabel));
  ...
  auto needs =
      rpl::combine(tr::lng_manage_peer_reactions_level(
                       lt_count, count->value() | tr::to_count(), lt_same_count,
                       std::move(countString), tr::rich),
  ...
  const auto wrap = inner->add(object_ptr<Ui::SlideWrap<Ui::FlatLabel>>(
      inner,
      object_ptr<Ui::FlatLabel>(inner, std::move(needs), st::boxDividerLabel),
      QMargins{0, st::normalFont->height, 0, 0}));
  wrap->toggleOn(count->value() |
                 rpl::map(rpl::mappers::_1 > allowedCustomReactions));
...
void EditAllowedReactionsBox(not_null<Ui::GenericBox *> box,
  ...
  const auto optionInitial =
      (allowed.type != AllowedReactionsType::Some)     ? Option::All
      : (allowed.some.empty() && !allowed.paidEnabled) ? Option::None
                                                       : Option::Some;
  ...
  const auto enabled =
      isGroup ? nullptr
              : container->add(object_ptr<Ui::SettingsButton>(
                    container.get(), tr::lng_manage_peer_reactions_enable(),
                    st::manageGroupNoIconButton.button));
  if (enabled) {
    enabled->toggleOn(rpl::single(optionInitial != Option::None));
    enabled->toggledValue() | rpl::on_next(
                                  [=](bool value) {
                                    state->selectorState =
                                        value ? SelectorState::Active
                                              : SelectorState::Disabled;
                                  },
                                  enabled->lifetime());
  }
  const auto group =
      std::make_shared<Ui::RadioenumGroup<Option>>(state->option.current());
  group->setChangedCallback([=](Option value) { state->option = value; });
  const auto addOption = [&](Option option, const QString &text) {
    if (!isGroup) {
      return;
    }
    container->add(object_ptr<Ui::Radioenum<Option>>(
                       container, group, option, text, st::settingsSendType),
  ...
  const auto about = [](Option option) {
    switch (option) {
    case Option::All:
      return tr::lng_manage_peer_reactions_all_about();
    case Option::Some:
      return tr::lng_manage_peer_reactions_some_about();
    case Option::None:
      return tr::lng_manage_peer_reactions_none_about();
    }
    Unexpected("Option value in EditAllowedReactionsBox.");
  };
  Ui::AddSkip(container);
  Ui::AddDividerText(container,
                     (isGroup ? (state->option.value() | rpl::map(about) |
                                 rpl::flatten_latest())
  ...
  const auto changed = [=](std::vector<Data::ReactionId> chosen,
    ...
    state->customCount =
        ranges::count_if(state->selected, &Data::ReactionId::custom);
    state->paidEnabled =
        ranges::contains(state->selected, Data::ReactionId::Paid());
    if (hardLimitHit) {
      box->uiShow()->showToast(tr::lng_manage_peer_reactions_limit(tr::now));
    }
  };
  changed(!selected.empty() ? std::move(selected)
          : !isGroup        ? CollectAvailableReactions(
                                  &args.navigation->parentController()->session())
                            : DefaultSelected(),
          {});
  Ui::AddSubsectionTitle(reactions,
                         enabled ? tr::lng_manage_peer_reactions_available()
                                 : tr::lng_manage_peer_reactions_some_title(),
                         st::manageGroupReactionsFieldPadding);
  reactions->add(AddReactionsSelector(
                     reactions,
                     {
  ...
  if (!isGroup) {
    AddReactionsText(container, args.navigation, args.allowedCustomReactions,
                     state->customCount.value(), args.askForBoosts);
    ...
    const auto activeColor = std::make_optional(st::windowActiveTextFg->c);
    const auto inner = wrap->entity();
    Ui::AddSkip(inner);
    Ui::AddSubsectionTitle(inner, tr::lng_manage_peer_reactions_max_title(),
                           st::manageGroupReactionsMaxSubtitlePadding);
    Ui::AddSkip(inner);
    const auto line =
        inner->add(object_ptr<Ui::RpWidget>(inner), st::boxRowPadding);
    Ui::AddSkip(inner);
    Ui::AddSkip(inner);
    const auto left = Ui::CreateChild<Ui::FlatLabel>(line, QString::number(1),
                                                     st::defaultFlatLabel);
    const auto center =
        Ui::CreateChild<Ui::FlatLabel>(line, st::defaultFlatLabel);
    const auto right = Ui::CreateChild<Ui::FlatLabel>(
        line, QString::number(max), st::defaultFlatLabel);
    const auto slider =
        Ui::CreateChild<Ui::MediaSlider>(line, st::settingsScale);
    rpl::combine(line->sizeValue(), left->sizeValue(), center->sizeValue(),
                 right->sizeValue()) |
        rpl::on_next(
            [=](const QSize &s, const QSize &leftSize, const QSize &centerSize,
                const QSize &rightSize) {
              const auto sliderHeight = st::settingsScale.seekSize.height();
              line->resize(line->width(), leftSize.height() + sliderHeight * 2);
              {
                const auto r = line->rect();
                slider->setGeometry(0, r.height() - sliderHeight * 1.5,
                                    r.width(), sliderHeight);
              }
              left->moveToLeft(0, 0);
    ...
    const auto updateLabels = [=](int limit) {
      left->setTextColorOverride((limit <= 1) ? activeColor : inactiveColor);

      center->setText(
          tr::lng_manage_peer_reactions_max_slider(tr::now, lt_count, limit));
      center->setTextColorOverride(activeColor);

      right->setTextColorOverride((limit >= max) ? activeColor : inactiveColor);
    ...
    slider->setPseudoDiscrete(
        max, [=](int index) { return index + 1; }, current, updateLabels,
        updateLabels);
    updateLabels(current);

    wrap->toggleOn(
        rpl::single(optionInitial != Option::None) |
        rpl::then(state->selectorState.value() |
    ...
    const auto paid = inner->add(object_ptr<Ui::SettingsButton>(
        inner, tr::lng_manage_peer_reactions_paid(),
        st::manageGroupNoIconButton.button));
    paid->toggleOn(state->paidEnabled.value());
    paid->toggledValue() |
        rpl::on_next([=](bool value) { state->paidEnabled = value; },
                     paid->lifetime());
    Ui::AddSkip(inner);
  ...
  const auto collect = [=] {
    ...
    auto simple =
        all | ranges::views::transform(&Data::Reaction::id) | ranges::to_vector;
    ranges::sort(some);
    ranges::sort(simple);
    result.type = isGroup            ? (state->option.current() != Option::All
                                            ? AllowedReactionsType::Some
                                            : AllowedReactionsType::All)
                  : (some == simple) ? AllowedReactionsType::Default
                                     : AllowedReactionsType::Some;
    return result;
  };
  ...
  box->addButton(tr::lng_settings_save(), [=] {
    ...
    if (!isGroup) {
      const auto custom =
          ranges::count_if(result.some, &Data::ReactionId::custom);
...
void SaveAllowedReactions(not_null<PeerData *> peer,
                          const Data::AllowedReactions &allowed) {
  auto ids = allowed.some | ranges::views::transform(Data::ReactionToMTP) |
             ranges::to<QVector<MTPReaction>>;

  using Flag = MTPmessages_SetChatAvailableReactions::Flag;
  using Type = Data::AllowedReactionsType;
  const auto updated =
      (allowed.type != Type::Some)
          ? MTP_chatReactionsAll(
                MTP_flags((allowed.type == Type::Default)
  ...
  peer->session()
      .api()
      .request(MTPmessages_SetChatAvailableReactions(
          MTP_flags(Flag() | (maxCount ? Flag::f_reactions_limit : Flag()) |
                    (editPaidEnabled ? Flag::f_paid_enabled : Flag())),
          peer->input(), updated, MTP_int(maxCount), MTP_bool(paidEnabled)))
