...
rpl::producer<TextWithEntities> Text3() {
  return tr::lng_about_text3(
      lt_faq_link, tr::lng_about_text3_faq(tr::url(telegramFaqLink())),
      tr::marked);
}
...
void AboutBox(not_null<Ui::GenericBox *> box) {
  box->setTitle(u"Lotusgram"_q);

  auto layout = box->verticalLayout();

  const auto version = layout->add(
      object_ptr<Ui::LinkButton>(
          box, tr::lng_about_version(tr::now, lt_version, currentVersionText()),
          st::aboutVersionLink),
      QMargins(st::boxRowPadding.left(), -st::lineWidth * 3,
  ...
  version->setClickedCallback([=] {
    if (cRealAlphaVersion()) {
      ...
      url = url.arg(
          u"talpha%1_%2"_q.arg(cRealAlphaVersion())
              .arg(Core::countAlphaVersionSignature(cRealAlphaVersion())));

      QGuiApplication::clipboard()->setText(url);

      box->getDelegate()->show(
          Ui::MakeInformBox("The link to the current private alpha "
...
void ArchiveHintBox(not_null<Ui::GenericBox *> box, bool unarchiveOnNewMessage,
                    Fn<void()> onUnarchive) {
  ...
  Ui::AddSkip(content);
  Ui::AddSkip(content);
  box->addRow(object_ptr<Ui::FlatLabel>(content, tr::lng_archive_hint_title(),
                                        st::boxTitle),
              style::al_top);
  Ui::AddSkip(content);
  Ui::AddSkip(content);
  {
    const auto label = box->addRow(object_ptr<Ui::FlatLabel>(
        content,
        (unarchiveOnNewMessage ? tr::lng_archive_hint_about_unmuted
    ...
    label->setLink(
        1, std::make_shared<GenericClickHandler>([=](ClickContext context) {
  ...
  Ui::AddSkip(content);
  {
    ...
    addEntry(tr::lng_archive_hint_section_1(),
             tr::lng_archive_hint_section_1_info(), st::menuIconArchive);
    Ui::AddSkip(content);
    Ui::AddSkip(content);
    addEntry(tr::lng_archive_hint_section_2(),
             tr::lng_archive_hint_section_2_info(), st::menuIconStealth);
    Ui::AddSkip(content);
    Ui::AddSkip(content);
    addEntry(tr::lng_archive_hint_section_3(),
  ...
  Ui::AddSkip(content);
  {
    ...
    auto button = object_ptr<Ui::RoundButton>(
        box, tr::lng_archive_hint_button(), st::defaultActiveButton);
