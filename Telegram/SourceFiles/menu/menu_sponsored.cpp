...
[[nodiscard]] SponsoredPhrases PhrasesForMessage(FullMsgId fullId) {
  return peerIsChannel(fullId.peer) ? SponsoredPhrases::Channel
...
void AboutBox(not_null<Ui::GenericBox *> box,
  ...
  Ui::AddSkip(content);
  Ui::AddSkip(content);
  box->addRow(object_ptr<Ui::FlatLabel>(content,
  ...
  {
    const auto padding =
        QMargins(st::settingsButton.padding.left(), st::boxRowPadding.top(),
                 st::boxRowPadding.right(), st::boxRowPadding.bottom());
    const auto addEntry = [&](rpl::producer<QString> title,
                              rpl::producer<TextWithEntities> about,
                              const style::icon &icon) {
      const auto top =
          content->add(object_ptr<Ui::FlatLabel>(content, std::move(title),
                                                 st::channelEarnSemiboldLabel),
                       padding);
      Ui::AddSkip(content, st::channelEarnHistoryThreeSkip);
      const auto label = content->add(
          object_ptr<Ui::FlatLabel>(content, std::move(about),
                                    st::channelEarnHistoryRecipientLabel),
          padding);
      const auto left =
          Ui::CreateChild<Ui::RpWidget>(box->verticalLayout().get());
      left->paintRequest() | rpl::on_next(
                                 [=] {
                                   auto p = Painter(left);
      ...
      top->geometryValue() |
          rpl::on_next(
              [=](const QRect &g) {
                left->moveToLeft((g.left() - left->width()) / 2,
                                 g.top() + st::channelEarnHistoryThreeSkip);
              },
              left->lifetime());
      return label;
    };
    addEntry(tr::lng_sponsored_revenued_info1_title(),
             (isChannel ? tr::lng_sponsored_revenued_info1_description
              : isSearch
                  ? tr::lng_sponsored_revenued_info1_search_description
                  : tr::lng_sponsored_revenued_info1_bot_description)(tr::rich),
             st::sponsoredAboutPrivacyIcon);
    if (!isSearch) {
      Ui::AddSkip(content);
      Ui::AddSkip(content);
      addEntry((isChannel ? tr::lng_sponsored_revenued_info2_title
                          : tr::lng_sponsored_revenued_info2_bot_title)(),
               (isChannel ? tr::lng_sponsored_revenued_info2_description
    ...
    Ui::AddSkip(content);
    auto link =
        tr::lng_settings_privacy_premium_link() | rpl::map([](QString t) {
          return tr::link(std::move(t), u"internal:"_q);
        });
    addEntry(
        tr::lng_sponsored_revenued_info3_title(),
        (isChannel ? tr::lng_sponsored_revenued_info3_description(
                         lt_count, rpl::single(float64(levels)), lt_link,
                         std::move(link), tr::rich)
         : isSearch
             ? tr::lng_sponsored_revenued_info3_search_description(
                   ...
                   tr::lng_sponsored_revenued_info3_search_link(
                       ...
                       rpl::map([](TextWithEntities &&link) {
                         return Ui::Text::Wrapped(std::move(link),
                                                  EntityType::CustomUrl,
                                                  u"internal:"_q);
                       }),
                   tr::rich)
             : tr::lng_sponsored_revenued_info3_bot_description(
                   lt_link, std::move(link), tr::rich)),
  ...
  Ui::AddSkip(content);
  {
    box->addRow(
        object_ptr<Ui::FlatLabel>(
            content, tr::lng_sponsored_revenued_footer_title(), st::boxTitle),
        style::al_top);
  }
  Ui::AddSkip(content);
  {
    const auto arrow = Ui::Text::IconEmoji(&st::textMoreIconEmoji);
    const auto available = box->width() - rect::m::sum::h(st::boxRowPadding);
    box
        ->addRow(object_ptr<Ui::FlatLabel>(
            content,
            (isChannel  ? tr::lng_sponsored_revenued_footer_description
             : isSearch ? tr::lng_sponsored_revenued_footer_search_description
                        : tr::lng_sponsored_revenued_footer_bot_description)(
                lt_link,
                tr::lng_channel_earn_about_link(lt_emoji, rpl::single(arrow),
                                                tr::rich) |
  ...
  Ui::AddSkip(content);
  {
    ...
    box->setStyle(st);
    auto button = object_ptr<Ui::RoundButton>(box, tr::lng_box_ok(),
                                              st::defaultActiveButton);
    button->setTextTransform(Ui::RoundButtonTextTransform::ToUpper);
    button->resizeToWidth(box->width() - st.buttonPadding.left() -
  ...
  if (!isChannel) {
    const auto top = Ui::CreateChild<Ui::IconButton>(box, st::infoTopBarMenu);
    box->widthValue() |
        rpl::on_next(
            [=](int width) {
              top->raise();
              top->moveToLeft(
                  width - top->width() - st::defaultScrollArea.width, 0);
            },
            top->lifetime());
    using MenuPtr = base::unique_qptr<Ui::PopupMenu>;
    const auto menu = top->lifetime().make_state<MenuPtr>();
    top->setClickedCallback([=] {
      *menu = base::make_unique_q<Ui::PopupMenu>(box->window(),
      ...
      raw->animatePhaseValue() |
          rpl::on_next(
              [=](Ui::PopupMenu::AnimatePhase phase) {
                top->setForceRippled(
                    false || phase == Ui::PopupMenu::AnimatePhase::Shown ||
                    phase == Ui::PopupMenu::AnimatePhase::StartShow);
              },
              top->lifetime());
      raw->setDestroyedCallback([=] { top->setForceRippled(false); });
      FillSponsored(Ui::Menu::CreateAddActionCallback(menu->get()), show,
                    phrases, details, report, {.skipAbout = true});
      const auto global =
          top->mapToGlobal(QPoint(top->width() / 4 * 3, top->height() / 2));
      raw->setForcedOrigin(Ui::PanelAnimation::Origin::TopRight);
      raw->popup(QPoint(global.x(), std::max(global.y(), QCursor::pos().y())));
      return true;
    });
  }
}

void ShowReportSponsoredBox(std::shared_ptr<ChatHelpers::Show> show,
                            Data::SponsoredReportAction report) {
      ...
      if (!result.options.empty()) {
        show->show(Box([=](not_null<Ui::GenericBox *> box) {
          ...
          for (const auto &option : result.options) {
            const auto button = Ui::AddReportOptionButton(box->verticalLayout(),
                                                          option.text, nullptr);
            button->setClickedCallback(
                [=] { repeatRequest(repeatRequest, option.id); });
          }
          if (!id.isNull()) {
            box->addLeftButton(tr::lng_create_group_back(),
                               [=] { box->closeBox(); });
          } else {
            ...
            container->add(object_ptr<Ui::DividerLabel>(
                ...
                object_ptr<Ui::FlatLabel>(
                    ...
                    tr::lng_report_sponsored_reported_learn(
                        lt_link, rpl::single(guideLink), tr::marked),
                    st::boxDividerLabel),
                st::defaultBoxDividerLabelPadding));
          }
          box->addButton(tr::lng_close(), [=] { show->hideLayer(); });
        }));
      } else {
        ...
        case Data::SponsoredReportResult::FinalStep::Reported: {
          auto text = tr::lng_report_sponsored_reported(tr::now, lt_link,
                                                        guideLink, tr::marked);
          show->showToast({
              .text = std::move(text),
...
void FillSponsored(const Ui::Menu::MenuCallback &addAction,
  ...
  if (!settings.skipInfo && !info.empty()) {
    auto fillSubmenu = [&](not_null<Ui::PopupMenu *> menu) {
      const auto allText =
          ranges::accumulate(info, TextWithEntities(),
      ...
      for (const auto &i : info) {
        auto item = base::make_unique_q<Ui::Menu::MultilineAction>(
            menu->menu(), dark ? st::storiesMenu : st::defaultMenu,
            (dark ? st::historySponsorInfoItemDark
                  : st::historySponsorInfoItem),
            st::historyHasCustomEmojiPosition, base::duplicate(i));
    ...
    addAction({
        ...
        .handler = nullptr,
        .icon = (dark ? &st::mediaMenuIconChannel : &st::menuIconChannel),
        .fillSubmenu = std::move(fillSubmenu),
    });
    addAction({
        .separatorSt =
            (dark ? &st::mediaviewMenuSeparator : &st::expandedMenuSeparator),
        .isSeparator = true,
    });
  }
  ...
  addAction(
      tr::lng_sponsored_hide_ads(tr::now),
      [=] {
        if (session->premium()) {
...
void FillSponsored(const Ui::Menu::MenuCallback &addAction,
                   std::shared_ptr<ChatHelpers::Show> show,
                   const FullMsgId &fullId, SponsoredMenuSettings settings) {
  const auto session = &show->session();
  FillSponsored(addAction, show, PhrasesForMessage(fullId),
                session->sponsoredMessages().lookupDetails(fullId),
                session->sponsoredMessages().createReportCallback(fullId),
                settings);
}

void ShowSponsored(not_null<Ui::RpWidget *> parent,
                   std::shared_ptr<ChatHelpers::Show> show,
                   const FullMsgId &fullId) {
  const auto menu =
      Ui::CreateChild<Ui::PopupMenu>(parent.get(), st::popupMenuWithIcons);

  FillSponsored(Ui::Menu::CreateAddActionCallback(menu), show, fullId);
...
void ShowSponsoredAbout(std::shared_ptr<ChatHelpers::Show> show,
  ...
  show->showBox(Box([=](not_null<Ui::GenericBox *> box) {
    AboutBox(box, show, PhrasesForMessage(fullId),
             session->sponsoredMessages().lookupDetails(fullId),
             session->sponsoredMessages().createReportCallback(fullId));
  }));
}
