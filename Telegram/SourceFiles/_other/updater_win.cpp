...
void fullClearPath(const wstring &dir) {
  ...
  if (res)
    writeLog(L"Error: failed to clear path! :(");
}
  ...
  do {
    ...
    do {
      ...
      if (fname.substr(0, tdataDir.size()) == tdataDir &&
        ...
        if (equal(
                tofname,
                updaterName)) { // bad update - has Updater.exe - delete all dir
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prevInstance,
  ...
  if (args) {
    ...
    if (needupdate)
      writeLog(L"Need to update!");
    if (autostart)
      writeLog(L"From autostart!");
    if (writeprotected)
      writeLog(L"Write Protected folder!");
