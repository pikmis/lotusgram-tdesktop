...
[[nodiscard]] std::string SerializeClientData(const QByteArray &challenge,
                                              const QString &type) {
...
std::optional<RegisterData>
DeserializeRegisterData(const QByteArray &jsonData) {
  ...
  data.user.id = QByteArray::fromBase64(user["id"].toString().toUtf8());
  data.user.name = user["name"].toString();
  data.user.displayName = user["displayName"].toString();

  data.challenge =
      QByteArray::fromBase64(publicKey["challenge"].toString().toUtf8(),
...
std::optional<LoginData> DeserializeLoginData(const QByteArray &jsonData) {
  auto doc = QJsonDocument::fromJson(jsonData);
  if (!doc.isObject()) {
    return std::nullopt;
  }
  ...
  if (publicKey.contains("allowCredentials")) {
    ...
    for (const auto &cred : allowList) {
      ...
      credential.id = QByteArray::fromBase64(credObj["id"].toString().toUtf8(),
                                             QByteArray::Base64UrlEncoding);
