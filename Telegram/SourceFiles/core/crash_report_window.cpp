...
PreLaunchWindow::PreLaunchWindow(QString title) {
  ...
  setWindowTitle(title.isEmpty() ? u"Lotusgram"_q : title);
...
NotStartedWindow::NotStartedWindow() : _label(this), _log(this), _close(this) {
  _label.setText(
      u"Could not start Telegram Desktop!\nYou can see complete log below:"_q);
...
void NotStartedWindow::resizeEvent(QResizeEvent *e) {
  int padding = _size;
  _label.setGeometry(padding, padding, width() - 2 * padding,
                     _label.sizeHint().height());
  _log.setGeometry(padding, padding * 2 + _label.sizeHint().height(),
                   width() - 2 * padding,
                   height() - 4 * padding - _label.height() - _close.height());
  _close.setGeometry(width() - padding - _close.width(),
                     height() - padding - _close.height(), _close.width(),
                     _close.height());
}

LastCrashedWindow::UpdaterData::UpdaterData(QWidget *buttonParent)
    : check(buttonParent), skip(buttonParent, false) {}

LastCrashedWindow::LastCrashedWindow(const QByteArray &crashdump,
                                     Fn<void()> launch)
    : _dumpraw(crashdump), _label(this), _pleaseSendReport(this),
      _yourReportName(this), _minidump(this), _report(this), _send(this),
      _sendSkip(this, false), _networkSettings(this), _continue(this),
      _showReport(this), _saveReport(this), _getApp(this),
      _includeUsername(this), _reportText(QString::fromUtf8(crashdump)),
      _reportShown(false), _reportSaved(false),
      _sendingState(crashdump.isEmpty() ? SendingNoReport : SendingUpdateCheck),
      _updating(this), _updaterData(Core::UpdaterDisabled()
                                        ? nullptr
  ...
  if (_sendingState != SendingNoReport) {
    ...
    if (_minidumpFull.isEmpty()) {
      ...
      QDateTime maxDumpModified,
          workingModified =
              QFileInfo(cWorkingDir() + u"tdata/working"_q).lastModified();
      ...
      for (int32 i = 0, l = list.size(); i < l; ++i) {
        ...
        if (name.endsWith(qstr(".dmp"))) {
          ...
          if (maxDump.isEmpty() ||
              qAbs(workingModified.secsTo(modified)) <
                  qAbs(workingModified.secsTo(maxDumpModified))) {
            maxDump = name;
      ...
      if (!maxDump.isEmpty() &&
          qAbs(workingModified.secsTo(maxDumpModified)) < 10) {
        _minidumpName = maxDump;
        _minidumpFull = maxDumpFull;
      }
    }
    if (_minidumpName.isEmpty()) { // currently don't accept crash reports
                                   // without dumps from google libraries
      _sendingState = SendingNoReport;
    } else {
      _minidump.setText(
          u"+ %1 (%2 KB)"_q.arg(_minidumpName).arg(dumpsize / 1024));
    }
  }
  if (_sendingState != SendingNoReport) {
    QString version = getReportField(qstr("version"), qstr("Version:"));
    QString current = cAlphaVersion() ? u"-%1"_q.arg(cAlphaVersion())
                                      : QString::number(AppVersion);
    if (version != current) { // currently don't accept crash reports from not
                              // current app version
      _sendingState = SendingNoReport;
    }
  }
  ...
  if (_updaterData) {
    ...
    connect(&_updaterData->check, &QPushButton::clicked,
            [=] { updateRetry(); });
  ...
  _yourReportName.setText(
      u"Crash ID: %1"_q.arg(QString(_minidumpName).replace(".dmp", "")));
  _yourReportName.setCursor(style::cur_text);
  _yourReportName.setTextInteractionFlags(Qt::TextSelectableByMouse);

  _includeUsername.setText(
      u"Include username @%1 as your contact info"_q.arg(_reportUsername));
...
QByteArray LastCrashedWindow::getCrashReportRaw() const {
  ...
  if (!_reportUsername.isEmpty() &&
      _includeUsername.checkState() != Qt::Checked) {
    result.replace((u"Username: "_q + _reportUsername).toUtf8(),
...
void LastCrashedWindow::excludeReportUsername() {
  ...
  _reportTextNoUsername =
      _reportUsername.isEmpty() ? _reportText : lines.join('\n');
}

QString LastCrashedWindow::getReportField(const QLatin1String &name,
    ...
    if (lines.at(i).trimmed().startsWith(prefix)) {
      ...
      if (name == qstr("version")) {
        if (data.endsWith(qstr(" alpha"))) {
          data = QString::number(
              -data.replace(QRegularExpression(u"[^\\d]"_q), "").toLongLong());
        } else {
          data = QString::number(
              data.replace(QRegularExpression(u"[^\\d]"_q), "").toLongLong());
        }
...
void LastCrashedWindow::sendReport() {
  ...
  _checkReply = _sendManager.get(QNetworkRequest(
      u"https://tdesktop.com/crash.php?act=query_report&apiid=%1&version=%2&dmp=%3&platform=%4"_q
          .arg(apiid, version,
               QString::number(minidumpFileName().isEmpty() ? 0 : 1),
               CrashReports::PlatformString())));

  connect(_checkReply, &QNetworkReply::errorOccurred,
          [=](QNetworkReply::NetworkError code) { sendingError(code); });
  connect(_checkReply, &QNetworkReply::finished, [=] { checkingFinished(); });
...
QString LastCrashedWindow::minidumpFileName() {
  ...
  if (dmpFile.exists() && dmpFile.size() > 0 &&
...
void LastCrashedWindow::checkingFinished() {
  ...
  LOG(("Crash report check for sending done, result: %1")
          .arg(QString::fromUtf8(result)));

  if (result == "Old") {
    _pleaseSendReport.setText(
        u"This report is about some old version of Telegram Desktop."_q);
    _sendingState = SendingTooOld;
    updateControls();
    return;
  } else if (result == "Unofficial") {
    _pleaseSendReport.setText(
        u"You use some custom version of Telegram Desktop."_q);
  ...
  reportPart.setHeader(
      QNetworkRequest::ContentDispositionHeader,
      QVariant(
          "form-data; name=\"report\"; filename=\"report.telegramcrash\""));
  ...
  if (!dmpName.isEmpty()) {
    ...
    if (file.open(QIODevice::ReadOnly)) {
      ...
      minidumpZip.openNewFile(dmpNameUtf.constData(), &zfi, nullptr, 0, nullptr,
                              0, nullptr, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
      ...
      if (minidumpZip.error() == ZIP_OK) {
        QHttpPart dumpPart;
        dumpPart.setHeader(QNetworkRequest::ContentTypeHeader,
                           QVariant("application/octet-stream"));
        dumpPart.setHeader(
            QNetworkRequest::ContentDispositionHeader,
            QVariant(
                u"form-data; name=\"dump\"; filename=\"%1\""_q.arg(zipName)));
        dumpPart.setBody(minidumpZip.result());
        multipart->append(dumpPart);

        _minidump.setText(u"+ %1 (%2 KB)"_q.arg(zipName).arg(
            minidumpZip.result().size() / 1024));
      }
    }
  }

  _sendReply = _sendManager.post(
      QNetworkRequest(u"https://tdesktop.com/crash.php?act=report"_q),
      multipart);
  multipart->setParent(_sendReply);

  connect(_sendReply, &QNetworkReply::errorOccurred,
          [=](QNetworkReply::NetworkError code) { sendingError(code); });
  connect(_sendReply, &QNetworkReply::finished, [=] { sendingFinished(); });
  connect(_sendReply, &QNetworkReply::uploadProgress,
  ...
  if (_updaterData) {
    ...
    if (_updaterData->state == UpdatingFail &&
      ...
      if (_updaterData->state == UpdatingCheck ||
          _sendingState == SendingFail || _sendingState == SendingProgress) {
        _networkSettings.show();
      } else {
      ...
      if (_updaterData->state == UpdatingNone ||
        ...
        if (_sendingState == SendingNoReport) {
          ...
          h += _showReport.height() + padding + _yourReportName.height() +
               padding;
          ...
          if (_sendingState == SendingTooOld ||
              _sendingState == SendingUnofficial) {
            ...
            if (_reportShown) {
              h += (_pleaseSendReport.height() * 12.5) + padding +
                   (_minidumpName.isEmpty() ? 0
                                            : (_minidump.height() + padding));
              _report.show();
              ...
              if (_reportSaved || _sendingState == SendingFail ||
                  _sendingState == SendingProgress ||
            ...
            if (_sendingState == SendingTooMany ||
                _sendingState == SendingDone) {
              _send.hide();
              _sendSkip.hide();
              _continue.show();
            } else {
              if (_sendingState == SendingProgress ||
                  _sendingState == SendingUploading) {
                _send.hide();
              } else {
                _send.show();
  ...
  QSize s(
      2 * padding +
          QFontMetrics(_label.font())
              .horizontalAdvance(
                  u"Last time Telegram Desktop was not closed properly."_q) +
...
void LastCrashedWindow::networkSettings() {
  ...
  const auto box = new NetworkSettingsWindow(
      this, proxy.host, proxy.port ? proxy.port : kDefaultProxyPort, proxy.user,
      proxy.password);
  box->saveRequests() | rpl::on_next(
                            [=](MTP::ProxyData &&data) {
...
void LastCrashedWindow::proxyUpdated() {
  if (_updaterData && ((_updaterData->state == UpdatingCheck) ||
                       (_updaterData->state == UpdatingFail &&
                        (_sendingState == SendingNoReport ||
                         _sendingState == SendingUpdateCheck)))) {
    Core::UpdateChecker checker;
...
void LastCrashedWindow::setDownloadProgress(qint64 ready, qint64 total) {
  ...
  qint64 readyTenthMb = (ready * 10 / (1024 * 1024)),
         totalTenthMb = (total * 10 / (1024 * 1024));
  QString readyStr = QString::number(readyTenthMb / 10) + '.' +
                     QString::number(readyTenthMb % 10);
  QString totalStr = QString::number(totalTenthMb / 10) + '.' +
                     QString::number(totalTenthMb % 10);
  QString res = u"Downloading update {ready} / {total} MB.."_q
                    .replace(qstr("{ready}"), readyStr)
...
void LastCrashedWindow::sendingFinished() {
  if (_sendReply) {
    ...
    LOG(("Crash report sending done, result: %1")
            .arg(QString::fromUtf8(result)));
...
void LastCrashedWindow::sendingProgress(qint64 uploaded, qint64 total) {
  ...
  if (total < 0) {
    _pleaseSendReport.setText(
        u"Sending crash report %1 KB..."_q.arg(uploaded / 1024));
  } else {
    _pleaseSendReport.setText(
        u"Sending crash report %1 / %2 KB..."_q.arg(uploaded / 1024)
            .arg(total / 1024));
  }
  updateControls();
}
...
void LastCrashedWindow::resizeEvent(QResizeEvent *e) {
  int padding = _size;
  _label.move(padding,
              padding + (_networkSettings.height() - _label.height()) / 2);

  _send.move(width() - padding - _send.width(),
             height() - padding - _send.height());
  if (_sendingState == SendingProgress || _sendingState == SendingUploading) {
    _sendSkip.move(width() - padding - _sendSkip.width(),
                   height() - padding - _sendSkip.height());
  } else {
    _sendSkip.move(width() - padding - _send.width() - padding -
                       _sendSkip.width(),
                   height() - padding - _sendSkip.height());
  }

  _updating.move(padding,
                 padding * 2 + _networkSettings.height() +
                     (_networkSettings.height() - _updating.height()) / 2);

  if (_updaterData) {
    _pleaseSendReport.move(
        padding, padding * 2 + _networkSettings.height() +
    ...
    _showReport.move(padding * 2 + _pleaseSendReport.width(),
                     padding * 2 + _networkSettings.height() +
                         _networkSettings.height() + padding);
    _yourReportName.move(padding,
                         _showReport.y() + _showReport.height() + padding);
    _includeUsername.move(padding, _yourReportName.y() +
                                       _yourReportName.height() + padding);
    _getApp.move((width() - _getApp.width()) / 2,
                 _showReport.y() + _showReport.height() + padding);
    ...
    if (_sendingState == SendingFail || _sendingState == SendingProgress) {
      ...
      _networkSettings.move(padding * 2 + _updating.width(),
                            padding * 2 + _networkSettings.height());
    }
    ...
    if (_updaterData->state == UpdatingCheck ||
      ...
      _updaterData->check.move(width() - padding - _updaterData->check.width(),
                               height() - padding -
                                   _updaterData->check.height());
      _updaterData->skip.move(width() - padding - _updaterData->check.width() -
                                  padding - _updaterData->skip.width(),
                              height() - padding - _updaterData->skip.height());
    }
  } else {
    _getApp.move((width() - _getApp.width()) / 2,
                 _updating.y() + _updating.height() + padding);

    _pleaseSendReport.move(
        padding, padding * 2 + _networkSettings.height() +
                     _networkSettings.height() + padding + _getApp.height() +
                     padding +
                     (_showReport.height() - _pleaseSendReport.height()) / 2);
    _showReport.move(padding * 2 + _pleaseSendReport.width(),
                     padding * 2 + _networkSettings.height() +
  ...
  if (_reportUsername.isEmpty()) {
    _report.setGeometry(
        padding, _yourReportName.y() + _yourReportName.height() + padding,
        width() - 2 * padding, _pleaseSendReport.height() * 12.5);
  } else {
    _report.setGeometry(
        padding, _includeUsername.y() + _includeUsername.height() + padding,
        width() - 2 * padding, _pleaseSendReport.height() * 12.5);
  }
  _minidump.move(padding, _report.y() + _report.height() + padding);
  _saveReport.move(_showReport.x(), _showReport.y());

  _continue.move(width() - padding - _continue.width(),
                 height() - padding - _continue.height());
}

NetworkSettingsWindow::NetworkSettingsWindow(QWidget *parent, QString host,
                                             quint32 port, QString username,
                                             QString password)
    : PreLaunchWindow(u"HTTP Proxy Settings"_q), _hostLabel(this),
      _portLabel(this), _usernameLabel(this), _passwordLabel(this),
      _hostInput(this), _portInput(this), _usernameInput(this),
      _passwordInput(this, true), _save(this), _cancel(this, false),
      _parent(parent) {
...
void NetworkSettingsWindow::resizeEvent(QResizeEvent *e) {
  ...
  _hostInput.setGeometry(_hostLabel.x(), _hostLabel.y() + _hostLabel.height(),
                         2 * _hostLabel.width(), _hostInput.height());
  _portLabel.move(padding + _hostInput.width() + padding, padding);
  _portInput.setGeometry(_portLabel.x(), _portLabel.y() + _portLabel.height(),
                         width() - padding - _portLabel.x(),
                         _portInput.height());
  _usernameLabel.move(padding, _hostInput.y() + _hostInput.height() + padding);
  _usernameInput.setGeometry(
      _usernameLabel.x(), _usernameLabel.y() + _usernameLabel.height(),
      (width() - 3 * padding) / 2, _usernameInput.height());
  _passwordLabel.move(padding + _usernameInput.width() + padding,
                      _usernameLabel.y());
  _passwordInput.setGeometry(
      _passwordLabel.x(), _passwordLabel.y() + _passwordLabel.height(),
      width() - padding - _passwordLabel.x(), _passwordInput.height());

  _save.move(width() - padding - _save.width(),
             height() - padding - _save.height());
...
void NetworkSettingsWindow::save() {
  ...
  _saveRequests.fire({
      .type = host.isEmpty() ? MTP::ProxyData::Type::None
...
void NetworkSettingsWindow::updateControls() {
  ...
  int padding = _size;
  int w = 2 * padding + _hostLabel.width() * 2 + padding +
          _portLabel.width() * 2 + padding;
  int h = padding + _hostLabel.height() + _hostInput.height() + padding +
          _usernameLabel.height() + _usernameInput.height() + padding +
          _save.height() + padding;
  if (w == width() && h == height()) {
    resizeEvent(0);
  } else {
    setGeometry(_parent->x() + (_parent->width() - w) / 2,
                _parent->y() + (_parent->height() - h) / 2, w, h);
  }
}
