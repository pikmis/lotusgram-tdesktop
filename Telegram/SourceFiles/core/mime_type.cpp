...
[[nodiscard]] base::flat_set<QString> SplitExtensions(const QString &joined) {
...
QStringList MimeType::globPatterns() const {
  switch (_type) {
  case Known::WebP:
    return QStringList(u"*.webp"_q);
  case Known::Tgs:
    return QStringList(u"*.tgs"_q);
  case Known::Tgv:
    return QStringList(u"*.tgv"_q);
  case Known::TDesktopTheme:
    return QStringList(u"*.tdesktop-theme"_q);
  case Known::TDesktopPalette:
    return QStringList(u"*.tdesktop-palette"_q);
  default:
    break;
  }
...
QString MimeType::filterString() const {
  switch (_type) {
  case Known::WebP:
    return u"WebP image (*.webp)"_q;
  case Known::Tgs:
    return u"Telegram sticker (*.tgs)"_q;
  case Known::Tgv:
    return u"Wallpaper pattern (*.tgv)"_q;
  case Known::TDesktopTheme:
    return u"Theme files (*.tdesktop-theme)"_q;
  case Known::TDesktopPalette:
    return u"Palette files (*.tdesktop-palette)"_q;
  default:
    break;
  }
...
QString MimeType::name() const {
  ...
  case Known::Tgs:
    return u"application/x-tgsticker"_q;
  case Known::Tgv:
    return u"application/x-tgwallpattern"_q;
  case Known::TDesktopTheme:
    return u"application/x-tdesktop-theme"_q;
  case Known::TDesktopPalette:
    return u"application/x-tdesktop-palette"_q;
  default:
    break;
  }
...
MimeType MimeTypeForFile(const QFileInfo &file) {
...

  {
    ...
    if (f.open(QIODevice::ReadOnly)) {
      ...
      if (magic.size() >= 12) {
        if (!memcmp(magic.constData(), "RIFF", 4) &&
            !memcmp(magic.constData() + 8, "WEBP", 4)) {
          return MimeType(MimeType::Known::WebP);
        }
      }
      f.close();
...
MimeType MimeTypeForData(const QByteArray &data) {
  if (data.size() >= 12) {
    if (!memcmp(data.constData(), "RIFF", 4) &&
        !memcmp(data.constData() + 8, "WEBP", 4)) {
      return MimeType(MimeType::Known::WebP);
    }
  }
...
bool IsMimeSticker(const QString &mime) {
  return (mime == u"image/webp"_q) || IsMimeStickerAnimated(mime);
}

bool IsMimeAcceptedForPhotoVideoAlbum(const QString &mime) {
  return (mime == u"image/jpeg"_q) || (mime == u"image/png"_q) ||
         (mime == u"video/mp4"_q) || (mime == u"video/quicktime"_q);
}

bool FileIsImage(const QString &name, const QString &mime) {
  return name.isEmpty() ? mime.toLower().startsWith(u"image/"_q)
                        : (DetectNameType(name) == NameType::Image);
}

std::shared_ptr<QMimeData>
ShareMimeMediaData(not_null<const QMimeData *> original) {
...
QList<QUrl> ReadMimeUrls(not_null<const QMimeData *> data) {
  return (data->hasUrls() && !IsImageFromFirefox(data))
             ? KUrlMimeData::urlsFromMimeData(data,
...
bool NameTypeAllowsThumbnail(NameType type) {
  return type == NameType::Image || type == NameType::Video ||
         type == NameType::Audio || type == NameType::Document ||
...
bool IsIpRevealingPath(const QString &filepath) {
  ...
  return ranges::binary_search(kExtensions,
