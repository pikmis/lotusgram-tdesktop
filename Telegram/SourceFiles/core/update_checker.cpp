...
private:
  ...
  parseMessage(const MTPmessages_Messages &result) const;
...
bool UnpackUpdate(const QString &filepath) {
  ...
  uchar sha1Buffer[20];
  bool goodSha1 = !memcmp(compressed.constData() + hSigLen,
                          hashSha1(compressed.constData() + hSigLen + hShaLen,
                                   compressedLen + hPropsLen + hOriginalSizeLen,
                                   sha1Buffer),
                          hShaLen);
  if (!goodSha1) {
...
HttpChecker::parseOldResponse(const QByteArray &response) const {
  ...
  return validateLatestUrl(availableVersion, isAvailableAlpha,
                           isAvailableAlpha ? url.mid(5) + "_{signature}"
                                            : url);
}

std::optional<QString>
HttpChecker::parseResponse(const QByteArray &response) const {
  auto bestAvailableVersion = 0ULL;
  auto bestIsAvailableAlpha = false;
...
Updater::Updater()
  ...
  checking() | rpl::on_next([=] { handleChecking(); }, _lifetime);
  progress() | rpl::on_next([=] { handleProgress(); }, _lifetime);
  failed() | rpl::on_next([=] { handleFailed(); }, _lifetime);
  ready() | rpl::on_next([=] { handleReady(); }, _lifetime);
  isLatest() | rpl::on_next([=] { handleLatest(); }, _lifetime);
}
...
void Updater::startImplementation(not_null<Implementation *> which,
  ...
  checker->ready() | rpl::on_next(
                         ...
                         checker->lifetime());
  checker->failed() |
      rpl::on_next([=] { checkerFail(which); }, checker->lifetime());
    ...
    if (const auto loader = _activeLoader.get()) {
      ...
      loader->ready() |
          rpl::on_next(
              ...
              loader->lifetime());
      loader->failed() |
          rpl::on_next([=] { _failed.fire({}); }, loader->lifetime());
...
#else                  // Q_OS_MAC
  // if the files in the directory are owned by user, while the directory is
  // not, update will still fail since it's not possible to remove files
...
QString countAlphaVersionSignature(uint64 version) { // duplicated in packer.cpp
  if (cAlphaPrivateKey().isEmpty()) {
    LOG(("Error: Trying to count alpha version signature without alpha private "
         "key!"));
    return QString();
  }
