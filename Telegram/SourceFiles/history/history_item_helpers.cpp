...
int ComputeSendingMessagesCount(not_null<History *> history,
  ...
  return result + (request.story ? 1 : 0) +
         (request.forward ? int(request.forward->size()) : 0);
}

Data::SendError GetErrorForSending(not_null<PeerData *> peer,
  ...
  const auto thread =
      topic ? not_null<Data::Thread *>(topic) : peer->owner().history(peer);
  if (request.story) {
  ...
  if (peer->slowmodeApplied()) {
    ...
    if (const auto history = peer->owner().historyLoaded(peer)) {
      if (!request.ignoreSlowmodeCountdown &&
          (history->latestSendingMessage() != nullptr) && (count > 0)) {
  ...
  if (const auto left = peer->slowmodeSecondsLeft()) {
    if (!request.ignoreSlowmodeCountdown) {
      return tr::lng_slowmode_enabled(tr::now, lt_left,
...
Data::SendError GetErrorForSending(not_null<Data::Thread *> thread,
                                   SendingErrorRequest request) {
  request.topicRootId = thread->topicRootId();
  return GetErrorForSending(thread->peer(), std::move(request));
}

Data::SendErrorWithThread
GetErrorForSending(const std::vector<not_null<Data::Thread *>> &threads,
                   SendingErrorRequest request) {
...
std::optional<SendPaymentDetails>
ComputePaymentDetails(not_null<PeerData *> peer, int messagesCount) {
  ...
  const auto known2 = user ? user->messageMoneyRestrictionsKnown()
...
bool SuggestPaymentDataReady(not_null<PeerData *> peer,
...
MakeSendErrorBox(const Data::SendErrorWithThread &error, bool withTitle) {
...
void ShowSendPaidConfirm(not_null<Window::SessionNavigation *> navigation,
                         not_null<PeerData *> peer, SendPaymentDetails details,
                         Fn<void()> confirmed, PaidConfirmStyles styles,
                         int suggestStarsPrice) {
  return ShowSendPaidConfirm(navigation->uiShow(), peer, details, confirmed,
                             styles, suggestStarsPrice);
}
...
void ShowSendPaidConfirm(std::shared_ptr<Main::SessionShow> show,
                         ...
                         Fn<void()> confirmed, PaidConfirmStyles styles,
                         int suggestStarsPrice) {
  ShowSendPaidConfirm(std::move(show), std::vector<not_null<PeerData *>>{peer},
                      details, confirmed, styles, suggestStarsPrice);
}

void ShowSendPaidConfirm(std::shared_ptr<Main::SessionShow> show,
                         const std::vector<not_null<PeerData *>> &peers,
                         SendPaymentDetails details, Fn<void()> confirmed,
                         PaidConfirmStyles styles, int suggestStarsPrice) {
  Expects(!peers.empty());

  const auto singlePeer =
      (peers.size() > 1) ? (PeerData *)nullptr : peers.front().get();
  ...
  const auto singlePeerStars =
      singlePeer ? singlePeer->starsPerMessageChecked() : 0;
  if (singlePeer) {
    const auto session = &singlePeer->session();
    const auto trusted = session->local().isPeerTrustedPayForMessage(
        singlePeerId, singlePeerStars);
    if (trusted) {
      check();
      return;
    }
  ...
  show->showBox(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    const auto proceed = [=](Fn<void()> close) {
      if (singlePeer && (*trust)->checked()) {
        const auto session = &singlePeer->session();
        session->local().markPeerTrustedPayForMessage(singlePeerId,
                                                      singlePeerStars);
      }
      check();
      close();
        ...
        box,
        {
            .text = (singlePeer
                         ? tr::lng_payment_confirm_text(
                               tr::now, lt_count, stars / messages, lt_name,
                               tr::bold(singlePeer->shortName()), tr::rich)
                         : (usersOnly ? tr::lng_payment_confirm_users
                                      : tr::lng_payment_confirm_chats)(
                               tr::now, lt_count, int(peers.size()), tr::rich))
                        .append(' ')
                        .append(tr::lng_payment_confirm_sure(
                            tr::now, lt_count, messages, lt_amount,
                            tr::lng_payment_confirm_amount(tr::now, lt_count,
                                                           stars, tr::rich),
                            tr::rich)),
            .confirmed = proceed,
            .confirmText = tr::lng_payment_confirm_button(
                lt_count, rpl::single(messages * 1.)),
    ...
    if (singlePeer) {
      ...
      *trust = box->addRow(
          object_ptr<Ui::Checkbox>(
              box, tr::lng_payment_confirm_dont_ask(tr::now), false,
...
bool SendPaymentHelper::check(not_null<Window::SessionNavigation *> navigation,
                              ...
                              Api::SendOptions options, int messagesCount,
                              Fn<void(int)> resend, PaidConfirmStyles styles) {
  return check(navigation->uiShow(), peer, options, messagesCount,
               std::move(resend), styles);
}
...
bool SendPaymentHelper::check(std::shared_ptr<Main::SessionShow> show,
  ...
  const auto checkSuggestPriceStars =
      (admin || suggest.ton) ? 0
                             : int(base::SafeRound(suggest.price().value()));
  const auto checkSuggestPriceTon =
      (!admin && suggest.ton) ? suggest.price() : CreditsAmount();
  ...
  if (!details || !suggestDetails) {
    ...
    if ((!details || !suggest.ton) && !peer->session().credits().loaded()) {
      peer->session().credits().loadedValue() | rpl::filter(rpl::mappers::_1) |
          rpl::take(1) |
          rpl::on_next(
              [=] {
    ...
    peer->session().changes().peerUpdates(peer,
    ...
    ShowSendPaidConfirm(
        show, peer, *details, [=] { resend(stars); }, styles,
        checkSuggestPriceStars);
    return false;
    ...
    MaybeRequestBalanceIncrease(show, required,
                                SmallBalanceForSuggest{broadcastId}, done);
    return false;
  }
...
void RequestDependentMessageItem(not_null<HistoryItem *> item, PeerId peerId,
                                 MsgId msgId) {
  ...
  history->session().api().requestMessageData(
      (peerId ? history->owner().peer(peerId) : history->peer), msgId, done);
}

void RequestDependentMessageStory(not_null<HistoryItem *> item, PeerId peerId,
                                  StoryId storyId) {
  const auto fullId = item->fullId();
  const auto history = item->history();
  const auto session = &history->session();
  ...
  history->owner().stories().resolve(
      {peerId ? peerId : history->peer->id, storyId}, done);
}
...
bool ShouldSendSilent(not_null<PeerData *> peer,
...
TextWithEntities DropDisallowedCustomEmoji(not_null<PeerData *> to,
  ...
  if (!allowSetId) {
    ...
    text.entities.erase(ranges::remove_if(text.entities, predicate),
                        text.entities.end());
  } else {
    const auto predicate = [&](const EntityInText &entity) {
...
ClickHandlerPtr JumpToMessageClickHandler(not_null<HistoryItem *> item,
  ...
  return JumpToMessageClickHandler(item->history()->peer, item->id, returnToId,
                                   std::move(highlight));
}

ClickHandlerPtr JumpToMessageClickHandler(not_null<PeerData *> peer,
                                          MsgId msgId, FullMsgId returnToId,
                                          MessageHighlightId highlight) {
  return std::make_shared<LambdaClickHandler>([=] {
    const auto separate = Core::App().separateWindowFor(peer);
    const auto controller = separate ? separate->sessionController()
...
ClickHandlerPtr JumpToStoryClickHandler(not_null<PeerData *> peer,
                                        StoryId storyId) {
  return std::make_shared<LambdaClickHandler>([=] {
    const auto separate = Core::App().separateWindowFor(peer);
    const auto controller = separate ? separate->sessionController()
  ...
  return std::make_shared<LambdaClickHandler>([=](ClickContext context) {
    ...
    if (const auto controller = my.sessionWindow.get()) {
      ...
      if (session.premium()) {
        ...
        session.sponsoredMessages().createReportCallback(my.itemId).callback(
            Result::Id("-1"), [](const auto &) {});
      } else {
...
ClickHandlerPtr ReportSponsoredClickHandler(not_null<HistoryItem *> item) {
  return std::make_shared<LambdaClickHandler>([=](ClickContext context) {
    const auto my = context.other.value<ClickHandlerContext>();
    if (const auto controller = my.sessionWindow.get()) {
      Menu::ShowSponsored(controller->widget(), controller->uiShow(),
                          item->fullId());
    }
  });
}
...
MessageFlags FlagsFromMTP(MsgId id, MTPDmessage::Flags flags,
  ...
  return localFlags | (IsServerMsgId(id) ? Flag::HistoryEntry : Flag()) |
         ...
         ((flags & MTP::f_quick_reply_shortcut_id) ? Flag::ShortcutMessage
                                                   : Flag()) |
         ...
         ((flags & MTP::f_video_processing_pending) ? Flag::EstimatedDate
                                                    : Flag()) |
         ((flags & MTP::f_paid_suggested_post_ton) ? Flag::TonPaidSuggested
          : (flags & MTP::f_paid_suggested_post_stars)
              ? Flag::StarsPaidSuggested
              : Flag()) |
         ((flags & MTP::f_summary_from_language) ? Flag::CanBeSummarized
                                                 : Flag());
}

MessageFlags FlagsFromMTP(MsgId id, MTPDmessageService::Flags flags,
                          MessageFlags localFlags) {
  using Flag = MessageFlag;
  ...
  return localFlags | (IsServerMsgId(id) ? Flag::HistoryEntry : Flag()) |
         ...
         ((flags & MTP::f_from_id) ? Flag::HasFromId : Flag()) |
         ((flags & MTP::f_reply_to) ? Flag::HasReplyInfo : Flag()) |
         ((flags & MTP::f_reactions_are_possible) ? Flag::ReactionsAllowed
                                                  : Flag());
}
...
MTPMessageReplyHeader NewMessageReplyHeader(const Api::SendAction &action) {
  if (const auto replyTo = action.replyTo) {
    if (replyTo.storyId) {
      return MTP_messageReplyStoryHeader(peerToMTP(replyTo.storyId.peer),
    ...
    auto quoteEntities =
        Api::EntitiesToMTP(&action.history->session(), replyTo.quote.entities,
    ...
    return MTP_messageReplyHeader(
        MTP_flags(
            ...
            (replyTo.quote.empty() ? Flag()
                                   : (Flag::f_quote | Flag::f_quote_text |
                                      Flag::f_quote_offset)) |
        ...
        MTP_int(replyTo.messageId.msg), peerToMTP(externalPeerId),
        MTPMessageFwdHeader(), // reply_from
        MTPMessageMedia(),     // reply_media
        MTP_int(replyToTop), MTP_string(replyTo.quote.text), quoteEntities,
        MTP_int(replyTo.quoteOffset), MTP_int(replyTo.todoItemId),
...
MediaCheckResult CheckMessageMedia(const MTPMessageMedia &media) {
  ...
  return media.match(
      [](const MTPDmessageMediaEmpty &) { return Result::Good; },
      [](const MTPDmessageMediaContact &) { return Result::Good; },
      [](const MTPDmessageMediaGeo &data) {
        return data.vgeo().match(
            [](const MTPDgeoPoint &) { return Result::Good; },
      ...
      [](const MTPDmessageMediaPhoto &data) {
        ...
        return photo->match(
            [](const MTPDphoto &) { return Result::Good; },
      ...
      [](const MTPDmessageMediaDocument &data) {
        ...
        return document->match(
            [](const MTPDdocument &) { return Result::Good; },
            [](const MTPDdocumentEmpty &) { return Result::Empty; });
      },
      [](const MTPDmessageMediaWebPage &data) {
        return data.vwebpage().match(
            [](const MTPDwebPage &) { return Result::Good; },
            [](const MTPDwebPageEmpty &) { return Result::Good; },
            [](const MTPDwebPagePending &) { return Result::Good; },
            [](const MTPDwebPageNotModified &) { return Result::Unsupported; });
      },
      [](const MTPDmessageMediaGame &data) {
        return data.vgame().match(
            [](const MTPDgame &) { return Result::Good; });
      },
...
[[nodiscard]] CallId CallIdFromInput(const MTPInputGroupCall &data) {
  return data.match(
      [&](const MTPDinputGroupCall &data) { return data.vid().v; },
      [](const auto &) -> CallId {
...
ParseInvitedToCallUsers(not_null<HistoryItem *> item,
                        const QVector<MTPlong> &users) {
  auto &owner = item->history()->owner();
  return ranges::views::all(users) |
         ranges::views::transform(
             [&](const MTPlong &id) { return owner.user(id.v); }) |
         ranges::to_vector;
}

PreparedServiceText GenerateJoinedText(not_null<History *> history,
                                       not_null<UserData *> inviter,
                                       bool viaRequest) {
  if (inviter->id != history->session().userPeerId()) {
    auto result = PreparedServiceText();
    result.links.push_back(inviter->createOpenLink());
    result.text = (history->peer->isMegagroup() ? tr::lng_action_add_you_group
    ...
    result.text = tr::lng_action_user_joined(
        tr::now, lt_from, tr::link(self->name(), QString()), tr::marked);
    return result;
  }
  return {viaRequest ? tr::lng_action_you_joined_by_request_channel(tr::now,
                                                                    tr::marked)
                     : tr::lng_action_you_joined(tr::now, tr::marked)};
}

not_null<HistoryItem *> GenerateJoinedMessage(not_null<History *> history,
                                              TimeId inviteDate,
                                              not_null<UserData *> inviter,
                                              bool viaRequest) {
...
std::optional<bool> PeerHasThisCall(not_null<PeerData *> peer, CallId id) {
  ...
  return call                  ? std::make_optional(call->id() == id)
         : PeerCallKnown(peer) ? std::make_optional(false)
                               : std::nullopt;
}
[[nodiscard]] rpl::producer<bool>
PeerHasThisCallValue(not_null<PeerData *> peer, CallId id) {
  return peer->session().changes().peerFlagsValue(
             peer, Data::PeerUpdate::Flag::GroupCall) |
         rpl::filter([=] { return PeerCallKnown(peer); }) | rpl::map([=] {
           const auto call = peer->groupCall();
           return (call && call->id() == id);
         }) |
         rpl::distinct_until_changed() |
         rpl::take_while([=](bool hasThisCall) { return hasThisCall; }) |
         rpl::then(rpl::single(false));
}

[[nodiscard]] ClickHandlerPtr GroupCallClickHandler(not_null<PeerData *> peer,
                                                    CallId callId) {
  return std::make_shared<LambdaClickHandler>([=] {
    const auto call = peer->groupCall();
...
[[nodiscard]] MessageFlags FinalizeMessageFlags(not_null<History *> history,
                                                MessageFlags flags) {
...
[[nodiscard]] OnStackUsers
LookupRecentUnreadReactedUsers(not_null<HistoryItem *> item) {
...
[[nodiscard]] MessageFlags NewForwardedFlags(not_null<PeerData *> peer,
                                             PeerId from,
...
void ShowTrialTranscribesToast(int left, TimeId until) {
  ...
  const auto text =
      left ? tr::lng_audio_transcribe_trials_left(tr::now, lt_count, left,
                                                  lt_date, {date}, tr::marked)
           : tr::lng_audio_transcribe_trials_over(
                 tr::now, lt_date, tr::bold(date), lt_link,
