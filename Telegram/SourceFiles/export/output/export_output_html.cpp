...
auto CalculateThumbSize(int maxWidth, int maxHeight, int minWidth,
                        int minHeight, bool expandForRetina = false) {
  return [=](QSize largeSize) {
    ...
    const auto smallSize =
        (checkWidth > maxWidth || checkHeight > maxHeight)
            ? largeSize.scaled(maxWidth, maxHeight, Qt::KeepAspectRatio)
            : largeSize;
    const auto retinaSize =
        QSize(smallSize.width() & ~0x01, smallSize.height() & ~0x01);
...
QByteArray SerializeString(const QByteArray &value) {
  ...
  for (auto p = begin; p != end; ++p) {
    ...
    if (ch == '\n') {
      ...
      result.append(';');
    } else if (ch == char(0xE2) && (p + 2 < end) && *(p + 1) == char(0x80)) {
...
QByteArray MakeLinks(const QByteArray &value) {
  const auto domain = QByteArray("https://lotusgram.lol/");
  auto result = QByteArray();
  auto offset = 0;
  while (true) {
...
QByteArray JoinList(const QByteArray &separator,
...
QByteArray FormatCustomEmoji(const Data::Utf8String &custom_emoji,
  ...
  return (custom_emoji.isEmpty()
          ...
          : (custom_emoji == Data::TextPart::UnavailableEmoji())
              ...
              : ("<a href = \"" + (relativeLinkBase + custom_emoji).toUtf8() +
                 "\">")) +
         text + "</a>";
}

QByteArray FormatText(const std::vector<Data::TextPart> &data,
      ...
      ranges::views::all(data) |
          ranges::views::transform([&](const Data::TextPart &part) {
            ...
            case Type::Unknown:
              return text;
            case Type::Mention:
              return "<a href=\"" + internalLinksDomain.toUtf8() + text.mid(1) +
                     "\">" + text + "</a>";
            case Type::Hashtag:
              return "<a href=\"\" "
                     "onclick=\"return ShowHashtag(" +
                     SerializeString('"' + text.mid(1) + '"') + ")\">" + text +
                     "</a>";
            ...
            case Type::Pre:
              return "<pre>" + text + "</pre>";
            case Type::TextUrl:
              return "<a href=\"" + SerializeString(part.additional) + "\">" +
                     text + "</a>";
            ...
            case Type::BankCard:
              return text;
            case Type::Spoiler:
              return "<span class=\"spoiler hidden\" "
...
QByteArray FormatDateText(TimeId date) {
  ...
  return Data::NumberToString(parsed.day()) + ' ' + month(parsed.month()) +
...
QByteArray PeersMap::wrapPeerName(PeerId peerId) const {
  ...
  return result.isEmpty() ? QByteArray("Deleted") : SerializeString(result);
}

QByteArray PeersMap::wrapUserName(UserId userId) const {
  const auto result = user(userId).name();
  return result.isEmpty() ? QByteArray("Deleted Account")
...
QByteArray HtmlContext::pushTag(const QByteArray &tag,
...
QByteArray HtmlContext::popTag() {
  ...
  _tags.pop_back();
  return (data.block ? ("\n" + indent()) : QByteArray()) + "</" + data.name +
         ">" + (data.block ? "\n" : "");
}

QByteArray HtmlContext::indent() const { return QByteArray(_tags.size(), ' '); }

bool HtmlContext::empty() const { return _tags.empty(); }
...
public:
  ...
  pushTag(const QByteArray &tag,
          std::map<QByteArray, QByteArray> &&attributes = {});
  [[nodiscard]] QByteArray popTag();
  [[nodiscard]] QByteArray indent() const;

  [[nodiscard]] QByteArray pushDiv(const QByteArray &className,
                                   const QByteArray &style = {});

  [[nodiscard]] QByteArray pushUserpic(const UserpicData &userpic);
  [[nodiscard]] QByteArray pushListEntry(const UserpicData &userpic,
                                         const QByteArray &name,
                                         const QByteArray &details,
                                         const QByteArray &info,
                                         const QString &link = QString());
  [[nodiscard]] QByteArray
  pushStoriesListEntry(const StoryData &story, const QByteArray &name,
                       const QByteArrayList &details, const QByteArray &info,
                       const std::vector<Data::TextPart> &caption,
                       const QString &internalLinksDomain,
                       const QString &link = QString());
  [[nodiscard]] QByteArray pushAudioEntry(const QByteArray &name,
                                          const QByteArray &info,
                                          const QByteArrayList &details,
                                          const QByteArray &duration,
                                          const QString &link = QString());
  [[nodiscard]] QByteArray
  pushSessionListEntry(int apiId, const QByteArray &name,
                       const QByteArray &subname,
                       std::initializer_list<QByteArray> details,
                       const QByteArray &info = QByteArray());

  [[nodiscard]] QByteArray pushHeader(const QByteArray &header,
                                      const QString &path = QString());
  [[nodiscard]] QByteArray pushSection(const QByteArray &label,
                                       const QByteArray &type, int count,
                                       const QString &path);
  [[nodiscard]] QByteArray pushAbout(const QByteArray &text,
                                     bool withDivider = false);
  [[nodiscard]] QByteArray
  pushServiceMessage(int messageId, const Data::DialogInfo &dialog,
                     const QString &basePath, const QByteArray &text,
                     const Data::Photo *photo = nullptr);
  [[nodiscard]] std::pair<MessageInfo, QByteArray>
  pushMessage(const Data::Message &message, const MessageInfo *previous,
              const Data::DialogInfo &dialog, const QString &basePath,
              const PeersMap &peers, const QString &internalLinksDomain,
...
private:
  ...
  [[nodiscard]] QByteArray
  pushGenericListEntry(const QString &link, const UserpicData &userpic,
                       const QByteArray &name, const QByteArray &subname,
                       std::initializer_list<QByteArray> details,
                       const QByteArray &info);

  [[nodiscard]] bool messageNeedsWrap(const Data::Message &message,
                                      const MessageInfo *previous) const;
  [[nodiscard]] bool forwardedNeedsWrap(const Data::Message &message,
                                        const MessageInfo *previous) const;

  [[nodiscard]] MediaData
  prepareMediaData(const Data::Message &message, const QString &basePath,
                   const PeersMap &peers,
                   const QString &internalLinksDomain) const;
  [[nodiscard]] QByteArray
  pushMedia(const Data::Message &message, const QString &basePath,
            const PeersMap &peers, const QString &internalLinksDomain,
            Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink);
  [[nodiscard]] QByteArray pushGenericMedia(const MediaData &data);
  [[nodiscard]] QByteArray pushStickerMedia(const Data::Document &data,
                                            const QString &basePath);
  [[nodiscard]] QByteArray pushAnimatedMedia(const Data::Document &data,
                                             const QString &basePath);
  [[nodiscard]] QByteArray pushVideoFileMedia(const Data::Document &data,
                                              const QString &basePath);
  [[nodiscard]] QByteArray pushPhotoMedia(const Data::Photo &data,
                                          const QString &basePath);
  [[nodiscard]] QByteArray pushPoll(const Data::Poll &data,
                                    const QString &internalLinksDomain,
                                    const QString &relativeLinkBase);
  [[nodiscard]] QByteArray pushTodoList(const Data::TodoList &data,
                                        const QString &internalLinksDomain,
                                        const QString &relativeLinkBase);
  [[nodiscard]] QByteArray pushGiveaway(const PeersMap &peers,
                                        const Data::GiveawayStart &data);
  [[nodiscard]] QByteArray
  pushGiveaway(const PeersMap &peers, const Data::GiveawayResults &data,
               Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink);
...
QString WriteUserpicThumb(const QString &basePath, const QString &largePath,
                          const UserpicData &userpic,
                          const QString &postfix = "_thumb") {
  return Data::WriteImageThumb(basePath, largePath, userpic.pixelSize * 2,
                               userpic.pixelSize * 2, postfix);
}

HtmlWriter::Wrap::Wrap(const QString &path, const QString &base, Stats *stats)
    : _file(path, stats) {
...
bool HtmlWriter::Wrap::empty() const { return _file.empty(); }

QByteArray
HtmlWriter::Wrap::pushTag(const QByteArray &tag,
                          std::map<QByteArray, QByteArray> &&attributes) {
  return _context.pushTag(tag, std::move(attributes));
}

QByteArray HtmlWriter::Wrap::popTag() { return _context.popTag(); }

QByteArray HtmlWriter::Wrap::indent() const { return _context.indent(); }

QByteArray HtmlWriter::Wrap::pushDiv(const QByteArray &className,
                                     const QByteArray &style) {
  return style.isEmpty() ? _context.pushTag("div", {{"class", className}})
                         : _context.pushTag(
...
QByteArray HtmlWriter::Wrap::pushListEntry(const UserpicData &userpic,
                                           const QByteArray &name,
                                           const QByteArray &details,
                                           const QByteArray &info,
                                           const QString &link) {
  return pushGenericListEntry(link, userpic, name, {}, {details}, info);
}

QByteArray HtmlWriter::Wrap::pushStoriesListEntry(
    const StoryData &story, const QByteArray &name,
    const QByteArrayList &details, const QByteArray &info,
    const std::vector<Data::TextPart> &caption,
    const QString &internalLinksDomain, const QString &link) {
  ...
  if (!story.imageLink.isEmpty()) {
    const auto sizeStyle =
        "width: " + Data::NumberToString(kStoryThumbWidth) +
        "px; height: " + Data::NumberToString(kStoryThumbHeight) + "px";
    result.append(
        pushTag("img", {{"class", "story"},
                        {"style", sizeStyle},
...
QByteArray HtmlWriter::Wrap::pushAudioEntry(const QByteArray &name,
                                            const QByteArray &info,
                                            const QByteArrayList &details,
                                            const QByteArray &duration,
                                            const QString &link) {
...
QByteArray HtmlWriter::Wrap::pushSessionListEntry(
    int apiId, const QByteArray &name, const QByteArray &subname,
    std::initializer_list<QByteArray> details, const QByteArray &info) {
  const auto link = QString();
  auto userpic =
      UserpicData{Data::ApplicationColorIndex(apiId), kEntryUserpicSize};
  userpic.firstName = name;
  return pushGenericListEntry(link, userpic, name, subname, details, info);
}

QByteArray HtmlWriter::Wrap::pushGenericListEntry(
    const QString &link, const UserpicData &userpic, const QByteArray &name,
    const QByteArray &subname, std::initializer_list<QByteArray> details,
    const QByteArray &info) {
  auto result =
      link.isEmpty()
...
QByteArray HtmlWriter::Wrap::pushHeader(const QByteArray &header,
                                        const QString &path) {
...
QByteArray HtmlWriter::Wrap::pushSection(const QByteArray &header,
                                         const QByteArray &type, int count,
                                         const QString &link) {
...
QByteArray HtmlWriter::Wrap::pushAbout(const QByteArray &text,
                                       bool withDivider) {
  auto result = pushDiv(withDivider ? "page_about details with_divider"
                                    : "page_about details");
...
QByteArray HtmlWriter::Wrap::pushServiceMessage(int messageId,
                                                const Data::DialogInfo &dialog,
                                                const QString &basePath,
                                                const QByteArray &serialized,
                                                const Data::Photo *photo) {
  auto result =
      pushTag("div", {{"class", "message service"},
  ...
  if (photo) {
    ...
    userpic.imageLink = WriteUserpicThumb(basePath, userpic.largeLink, userpic);
...
auto HtmlWriter::Wrap::pushMessage(
    const Data::Message &message, const MessageInfo *previous,
    const Data::DialogInfo &dialog, const QString &basePath,
    const PeersMap &peers, const QString &internalLinksDomain,
    Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink)
  ...
  const auto serviceText = v::match(
      message.action.content,
      [&](const ActionChatCreate &data) {
        return serviceFrom + " created group &laquo;" +
               SerializeString(data.title) + "&raquo;" +
               (data.userIds.empty()
                    ? QByteArray()
                    : " with members " + peers.wrapUserNames(data.userIds));
      },
      [&](const ActionChatEditTitle &data) {
        return isChannel ? ("Channel title changed to &laquo;" +
                            SerializeString(data.title) + "&raquo;")
                         : (serviceFrom + " changed group title to &laquo;" +
                            SerializeString(data.title) + "&raquo;");
      },
      [&](const ActionChatEditPhoto &data) {
        return isChannel ? QByteArray("Channel photo changed")
                         : (serviceFrom + " changed group photo");
      },
      [&](const ActionChatDeletePhoto &data) {
        return isChannel ? QByteArray("Channel photo removed")
                         : (serviceFrom + " removed group photo");
      },
      [&](const ActionChatAddUser &data) {
        return serviceFrom + " invited " + peers.wrapUserNames(data.userIds);
      },
      [&](const ActionChatDeleteUser &data) {
        return serviceFrom + " removed " + peers.wrapUserName(data.userId);
      },
      [&](const ActionChatJoinedByLink &data) {
        return serviceFrom + " joined group by link from " +
               peers.wrapUserName(data.inviterId);
      },
      [&](const ActionChannelCreate &data) {
        return "Channel &laquo;" + SerializeString(data.title) +
               "&raquo; created";
      },
      ...
      [&](const ActionChannelMigrateFrom &data) {
        return serviceFrom + " converted a basic group to this supergroup " +
               "&laquo;" + SerializeString(data.title) + "&raquo;";
      },
      [&](const ActionPinMessage &data) {
        return serviceFrom + " pinned " + wrapReplyToLink("this message");
      },
      [&](const ActionHistoryClear &data) {
        return QByteArray("History cleared");
      },
      [&](const ActionGameScore &data) {
        return serviceFrom + " scored " + NumberToString(data.score) + " in " +
      ...
      [&](const ActionPaymentSent &data) {
        ...
        auto result = "You have successfully transferred " + amount + " for " +
      ...
      [&](const ActionPhoneCall &data) { return QByteArray(); },
      [&](const ActionScreenshotTaken &data) {
        return serviceFrom + " took a screenshot";
      },
      [&](const ActionCustomAction &data) { return data.message; },
      [&](const ActionBotAllowed &data) {
        return data.attachMenu ? "You allowed this bot to message you "
               ...
               : data.app.isEmpty()
                   ...
                   : ("You allowed this bot to message you when you logged in "
                      "on " +
      ...
      [&](const ActionSecureValuesSent &data) {
        ...
        return "You have sent the following documents: " + SerializeList(list);
      },
      ...
      [&](const ActionGroupCall &data) {
        const auto durationText =
            (data.duration
                 ...
                 : QString())
                .toUtf8();
        return isChannel ? ("Voice chat" + durationText)
      ...
      [&](const ActionSetMessagesTTL &data) {
        const auto periodText = (data.period == 7 * 86400) ? "7 days"
                                : (data.period == 86400)   ? "24 hours"
                                                           : QByteArray();
        return isChannel
                   ? (data.period
                          ? "New messages will auto-delete in " + periodText
                          : "New messages will not auto-delete")
                   : (data.period ? (serviceFrom +
                                     " has set messages to auto-delete in " +
                                     periodText)
                                  : (serviceFrom +
      ...
      [&](const ActionSetChatTheme &data) {
        if (data.emoji.isEmpty()) {
          return isChannel ? "Channel theme was disabled"
                           : (serviceFrom + " disabled chat theme");
        }
        return isChannel
                   ? ("Channel theme was changed to " + data.emoji).toUtf8()
                   : (serviceFrom + " changed chat theme to " + data.emoji)
                         .toUtf8();
      },
      ...
      [&](const ActionWebViewDataSent &data) {
        return "You have just successfully transferred data from the &laquo;" +
               SerializeString(data.text) + "&raquo; button to the bot";
      },
      [&](const ActionGiftPremium &data) {
        if (!data.days || data.cost.isEmpty()) {
          return serviceFrom + " sent you a gift.";
        }
        return serviceFrom + " sent you a gift for " + data.cost +
               ": Telegram Premium for " + QString::number(data.days).toUtf8() +
               " days.";
      },
      [&](const ActionTopicCreate &data) {
        return serviceFrom + " created topic &laquo;" +
               SerializeString(data.title) + "&raquo;";
      },
      [&](const ActionTopicEdit &data) {
        auto parts = QList<QByteArray>();
        if (!data.title.isEmpty()) {
          parts.push_back("title to &laquo;" + SerializeString(data.title) +
                          "&raquo;");
        }
        if (data.iconEmojiId) {
      ...
      [&](const ActionSetChatWallPaper &data) {
        return serviceFrom +
               (data.same ? (" set " + wrapReplyToLink("the same background") +
                             " for this chat")
                          : " set a new background for this chat");
      },
      [&](const ActionGiftCode &data) {
        return data.unclaimed ? ("This is an unclaimed Telegram Premium for " +
      ...
      [&](const ActionGiveawayLaunch &data) {
        return serviceFrom +
               " just started a giveaway "
               "of Telegram Premium subscriptions to its followers.";
      },
      [&](const ActionGiveawayResults &data) {
        return !data.winners ? "No winners of the giveaway could be selected."
               ...
               : (data.credits && !data.unclaimed)
                   ? NumberToString(data.winners) +
                         " of the giveaway was randomly "
                         "selected by Telegram and received their prize."
                   : NumberToString(data.winners) +
                         " of the giveaway was randomly "
                         "selected by Telegram and received private messages "
                         "with "
                         "giftcodes.";
      },
      [&](const ActionBoostApply &data) {
        return serviceFrom + " boosted the group " +
      ...
      [&](const ActionPaymentRefunded &data) {
        const auto amount = FormatMoneyAmount(data.amount, data.currency);
        auto result =
            peers.wrapPeerName(data.peerId) + " refunded back " + amount;
        return result;
      },
      ...
      [&](const ActionPrizeStars &data) {
        return "You won a prize in a giveaway organized by " +
               ...
               QString::number(data.amount).toUtf8() + " Telegram Stars.";
      },
      [&](const ActionStarGift &data) {
        return serviceFrom + " sent you a gift of " +
               QByteArray::number(data.stars) + " Telegram Stars.";
      },
      [&](const ActionPaidMessagesRefunded &data) {
        auto result =
            message.out
                ? ("You refunded " + QString::number(data.stars).toUtf8() +
                   " Stars for " + QString::number(data.messages).toUtf8() +
                   " messages to " + peers.wrapPeerName(dialog.peerId))
                : (peers.wrapPeerName(dialog.peerId) + " refunded " +
                   QString::number(data.stars).toUtf8() + " Stars for " +
      ...
      [&](const ActionPaidMessagesPrice &data) {
        if (isChannel) {
          auto result =
              !data.broadcastAllowed
                  ? "Direct messages were disabled."
                  : ("Price per direct message changed to " +
                     QString::number(data.stars).toUtf8() + " Telegram Stars.");
          return result;
        }
      ...
      [&](const ActionTodoCompletions &data) {
        ...
        const auto list = [](const QByteArrayList &v) {
          return v.isEmpty() ? QByteArray()
                 : (v.size() > 1)
                     ? (v.mid(0, v.size() - 1).join(", ") + " and " + v.back())
                     : v.front();
        };
        if (completed.isEmpty() && !incompleted.isEmpty()) {
          return serviceFrom + " marked " + list(incompleted) +
                 " as not done yet in " + wrapReplyToLink("this todo list") +
                 ".";
        } else if (!completed.isEmpty() && incompleted.isEmpty()) {
          return serviceFrom + " marked " + list(completed) + " as done in " +
                 wrapReplyToLink("this todo list") + ".";
        }
        return serviceFrom + " marked " + list(completed) + " as done and " +
               list(incompleted) + " as not done yet in " +
               wrapReplyToLink("this todo list") + ".";
      },
      ...
      [&](const ActionPollAppendAnswer &data) {
        return serviceFrom + " added &quot;" + data.option +
               "&quot; to the poll.";
      },
      [&](const ActionPollDeleteAnswer &data) {
        return serviceFrom + " removed &quot;" + data.option +
               "&quot; from the poll.";
      },
      [&](const ActionSuggestedPostApproval &data) {
        return serviceFrom + (data.rejected ? " rejected " : " approved ") +
               "your suggested post" +
               (data.price
                    ? (", for " + QString::number(data.price.value()).toUtf8() +
                       (data.price.ton() ? " TON" : " stars"))
                    : "") +
               (data.scheduleDate ? (", " + FormatDateText(data.scheduleDate) +
                                     " at " + FormatTimeText(data.scheduleDate))
                                  : "") +
      ...
      [&](const ActionSuggestedPostRefund &data) {
        return QByteArray() +
               (data.payerInitiated
                    ...
                    : "The admin deleted the post early, the payment was "
                      "refunded.");
      },
      [&](const ActionSuggestBirthday &data) {
        return serviceFrom + " suggests to add a date of birth: " +
      ...
      [&](const ActionNoForwardsToggle &data) {
        return serviceFrom + (data.newValue ? " disabled sharing in this chat"
                                            : " enabled sharing in this chat");
      },
      [&](const ActionNoForwardsRequest &data) {
        return serviceFrom + " requested to enable sharing in this chat";
      },
      [&](const ActionNewCreatorPending &data) {
        return peers.wrapUserName(data.newCreatorId) +
               " will become the new main admin in 7 days if " + serviceFrom +
               " does not return";
      },
      [&](const ActionChangeCreator &data) {
        return serviceFrom + " made " + peers.wrapUserName(data.newCreatorId) +
               " the new main admin of the group";
      },
      [&](const ActionManagedBotCreated &data) {
        return serviceFrom + " created a bot " + peers.wrapUserName(data.botId);
      },
  ...
  if (!serviceText.isEmpty()) {
    ...
    const auto photo = v::is<ActionChatEditPhoto>(content)
                       ...
                       : v::is<ActionSuggestProfilePhoto>(content)
                           ...
                           : nullptr;
    return {info, pushServiceMessage(message.id, dialog, basePath, serviceText,
                                     photo)};
  }
  ...
  if (message.forwarded) {
    ...
    if (message.forwardedFromId) {
      FillUserpicNames(forwardedUserpic, peers.peer(message.forwardedFromId));
    } else {
  ...
  const auto className =
      wrap ? "message default clearfix" : "message default clearfix joined";
  auto block = pushTag("div", {{"class", className},
  ...
  if (wrap) {
    ...
    block.append(SerializeString(ComposeName(userpic, "Deleted Account")));
    if (!via.isEmpty() &&
        (!message.forwarded || message.showForwardedAsOriginal)) {
  ...
  if (showForwardedInfo) {
    ...
    if (forwardedWrap) {
      block.append(pushDiv("from_name"));
      block.append(
          SerializeString(ComposeName(forwardedUserpic, "Deleted Account")));
      if (!via.isEmpty()) {
        block.append(" via @" + via);
      }
      block.append(pushTag(
  ...
  if (!message.inlineButtonRows.empty()) {
    ...
    for (const auto &row : message.inlineButtonRows) {
      ...
      for (const auto &button : row) {
        ...
        const auto content =
            (!button.data.isEmpty() ? (u"Data: "_q + button.data + endline)
                                    : QString()) +
            (!button.forwardText.isEmpty()
                 ? (u"Forward text: "_q + button.forwardText + endline)
                 : QString()) +
            (u"Type: "_q + HistoryMessageMarkupButton::TypeToString(button));
        const auto link =
            (button.type == Type::Url) ? button.data : QByteArray();
        const auto onclick =
            (button.type != Type::Url)
        ...
        block.append(pushTag(
            "a",
            {
                ...
                onclick.isEmpty() ? Attribute() : Attribute{"onclick", onclick},
            }));
        block.append(pushTag("div"));
        ...
        if (&button != &row.back()) {
          block.append(
              pushTag("div", {{"class", "bot_button_column_separator"}}));
          block.append(popTag());
        }
      }
      block.append(popTag());
  ...
  if (!message.reactions.empty()) {
    ...
    for (const auto &reaction : message.reactions) {
      ...
      case Reaction::Type::CustomEmoji:
        block.append(
            FormatCustomEmoji(reaction.documentId, "\U0001F44B", _base));
        break;
      ...
      if (!reaction.recent.empty()) {
        ...
        for (const auto &recent : reaction.recent) {
          ...
          block.append(pushUserpic(UserpicData({
              ...
              .pixelSize = 20,
              .firstName =
                  peer.user() ? peer.user()->info.firstName : peer.name(),
              .lastName = peer.user() ? peer.user()->info.lastName : "",
              .tooltip = peer.name(),
          })));
        }
        block.append(popTag());
...
bool HtmlWriter::Wrap::messageNeedsWrap(const Data::Message &message,
                                        const MessageInfo *previous) const {
  if (!previous) {
    return true;
    ...
    return true;
             ...
             message.showForwardedAsOriginal !=
                 previous->showForwardedAsOriginal ||
...
QByteArray HtmlWriter::Wrap::pushMedia(
    const Data::Message &message, const QString &basePath,
    const PeersMap &peers, const QString &internalLinksDomain,
    Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink) {
  const auto data =
      prepareMediaData(message, basePath, peers, internalLinksDomain);
...
QByteArray HtmlWriter::Wrap::pushGenericMedia(const MediaData &data) {
  ...
  if (data.link.isEmpty()) {
    ...
    result.append(pushTag(
        "a", {{"class", "media clearfix pull_left block_link " + data.classes},
              {"href",
               (IsGlobalLink(data.link) ? data.link.toUtf8()
  ...
  if (data.thumb.isEmpty()) {
    ...
    result.append(pushTag("img", {{"class", "thumb pull_left"},
...
QByteArray HtmlWriter::Wrap::pushStickerMedia(const Data::Document &data,
                                              const QString &basePath) {
  using namespace Data;

  const auto &[thumb, size] =
      WriteImageThumb(basePath, data.file.relativePath,
                      CalculateThumbSize(kStickerMaxWidth, kStickerMaxHeight,
                                         kStickerMinWidth, kStickerMinHeight),
                      "PNG", -1);
  if (thumb.isEmpty()) {
  ...
  auto result = pushDiv("media_wrap clearfix");
  result.append(
      pushTag("a", {{"class", "sticker_wrap clearfix pull_left"},
                    {"href", relativePath(data.file.relativePath).toUtf8()}}));
  const auto sizeStyle = "width: " + NumberToString(size.width() / 2) +
                         "px; height: " + NumberToString(size.height() / 2) +
                         "px";
  result.append(pushTag("img", {{"class", "sticker"},
                                {"style", sizeStyle},
...
QByteArray HtmlWriter::Wrap::pushAnimatedMedia(const Data::Document &data,
                                               const QString &basePath) {
  using namespace Data;

  auto size = QSize(data.width, data.height);
  auto thumbSize =
      CalculateThumbSize(kPhotoMaxWidth, kPhotoMaxHeight, kPhotoMinWidth,
                         kPhotoMinHeight, true)(size);
  if (data.thumb.file.relativePath.isEmpty() ||
      data.file.relativePath.isEmpty() || !thumbSize.width() ||
      !thumbSize.height()) {
    auto generic = MediaData();
  ...
  auto result = pushDiv("media_wrap clearfix");
  result.append(
      pushTag("a", {{"class", "animated_wrap clearfix pull_left"},
                    {"href", relativePath(data.file.relativePath).toUtf8()}}));
  ...
  result.append(popTag());
  const auto sizeStyle =
      "width: " + NumberToString(thumbSize.width() / 2) +
      "px; height: " + NumberToString(thumbSize.height() / 2) + "px";
  result.append(pushTag(
      "img", {{"class", "animated"},
              {"style", sizeStyle},
...
QByteArray HtmlWriter::Wrap::pushVideoFileMedia(const Data::Document &data,
                                                const QString &basePath) {
  using namespace Data;
  ...
  auto result = pushDiv("media_wrap clearfix");
  result.append(
      pushTag("a", {{"class", "video_file_wrap clearfix pull_left"},
                    {"href", relativePath(data.file.relativePath).toUtf8()}}));
  result.append(pushDiv("video_play_bg"));
...
QByteArray HtmlWriter::Wrap::pushPhotoMedia(const Data::Photo &data,
                                            const QString &basePath) {
  using namespace Data;

  const auto &[thumb, size] =
      WriteImageThumb(basePath, data.image.file.relativePath,
                      CalculateThumbSize(kPhotoMaxWidth, kPhotoMaxHeight,
                                         kPhotoMinWidth, kPhotoMinHeight));
  if (thumb.isEmpty()) {
    auto generic = MediaData();
    generic.title = "Photo";
    generic.status =
        Ui::FormatImageSizeText(QSize(data.image.width, data.image.height))
            .toUtf8();
  ...
  result.append(pushTag(
      "a", {{"class", "photo_wrap clearfix pull_left"},
            {"href", relativePath(data.image.file.relativePath).toUtf8()}}));
  const auto sizeStyle = "width: " + NumberToString(size.width() / 2) +
                         "px; height: " + NumberToString(size.height() / 2) +
                         "px";
  result.append(pushTag("img", {{"class", "photo"},
                                {"style", sizeStyle},
                                {"src", relativePath(thumb).toUtf8()},
                                {"empty", ""}}));
  result.append(popTag());
...
QByteArray HtmlWriter::Wrap::pushPoll(const Data::Poll &data,
  ...
  result.append(
      FormatText(data.question, internalLinksDomain, relativeLinkBase));
  result.append(popTag());
  ...
  const auto details = [&](const Poll::Answer &answer) {
    if (!answer.votes) {
      ...
      return " <span class=\"details\">" + votes(answer.votes) + "</span>";
    }
    return " <span class=\"details\">" + votes(answer.votes) +
           ", chosen vote</span>";
  };
...
QByteArray HtmlWriter::Wrap::pushTodoList(const Data::TodoList &data,
                                          const QString &internalLinksDomain,
                                          const QString &relativeLinkBase) {
  using namespace Data;
...
QByteArray HtmlWriter::Wrap::pushGiveaway(const PeersMap &peers,
                                          const Data::GiveawayStart &data) {
  auto result = pushDiv("media_wrap clearfix");
  result.append(pushDiv("media_giveaway"));

  result.append(pushDiv("section_title bold"));
  result.append((data.quantity > 1) ? SerializeString("Giveaway Prizes")
                                    : SerializeString("Giveaway Prize"));
  result.append(popTag());

  {
    result.append(pushDiv("section_body"));
    result.append("<b>" + Data::NumberToString(data.quantity) + "</b> " +
...

  {
    ...
    if (const auto count = countries.size()) {
      ...
      for (auto i = 1; i != count; ++i) {
        united = ((i + 1 == count) ? u"%1 and %2"_q : u"%1, %2"_q)
                     .arg(united, countries[i]);
      }
      result.append(pushDiv("section_body"));
      result.append(SerializeString((u"from %1"_q).arg(united).toUtf8()));
      result.append(popTag());
    }
  }
  result.append(pushDiv("section_title bold"));
...
QByteArray HtmlWriter::Wrap::pushGiveaway(
    const PeersMap &peers, const Data::GiveawayResults &data,
    Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink) {
  auto result = pushDiv("media_wrap clearfix");
  result.append(pushDiv("media_giveaway"));

  result.append(pushDiv("section_title bold"));
  result.append((data.winnersCount > 1) ? SerializeString("Winners Selected!")
  ...
  result.append(
      ...
      " of the " + wrapMessageLink(data.launchId, "Giveaway") +
      " was randomly selected by Telegram.");
  result.append(popTag());

  result.append(pushDiv("section_title bold"));
  result.append((data.winnersCount > 1) ? SerializeString("Winners")
  ...
  const auto prize = [&, singleStar = (data.credits == 1)] {
    if (data.credits && data.winnersCount == 1) {
      return SerializeString("The winner received ") + "<b>" +
             Data::NumberToString(data.credits) + "</b>" +
             SerializeString(singleStar ? " Star." : " Stars.");
    } else if (data.credits && data.winnersCount > 1) {
      return SerializeString("All winners received ") + "<b>" +
             Data::NumberToString(data.credits) + "</b>" +
             SerializeString(singleStar ? " Star in total."
                                        : " Stars in total.");
...
MediaData HtmlWriter::Wrap::prepareMediaData(
    const Data::Message &message, const QString &basePath,
    const PeersMap &peers, const QString &internalLinksDomain) const {
  using namespace Data;
  ...
  if (const auto call = std::get_if<ActionPhoneCall>(&action.content)) {
    ...
    result.title =
        peers.peer(message.out ? message.peerId : message.selfId).name();
    result.status = [&] {
    ...
    if (call->duration > 0) {
      ...
      result.status += " (" + NumberToString(call->duration) + " seconds)";
    }
    return result;
  }
  ...
  v::match(
      ...
      [&](const Photo &data) {
        if (message.media.ttl) {
          ...
          result.status = data.id ? "Please view it on your mobile" : "Expired";
      ...
      [&](const Document &data) {
        ...
        if (data.isSticker) {
          ...
          result.title =
              (!data.songPerformer.isEmpty() && !data.songTitle.isEmpty())
      ...
      [&](const GeoPoint &data) {
        ...
        if (data.valid) {
          ...
          result.link = "https://maps.google.com/maps?q=" + coords +
                        "&ll=" + coords + "&z=16";
        }
      },
      [&](const Venue &data) {
      ...
      [&](const Game &data) {
        ...
        if (data.botId != 0 && !data.shortName.isEmpty()) {
          ...
          if (bot.isBot && !bot.username.isEmpty()) {
            const auto link = internalLinksDomain.toUtf8() + bot.username +
                              "?game=" + data.shortName;
            result.link = link;
            result.status = link;
          }
      ...
      [](const Poll &data) {}, [](const TodoList &data) {},
      [](const GiveawayStart &data) {}, [](const GiveawayResults &data) {},
      [&](const PaidMedia &data) {
        result.classes = "media_invoice";
        result.status = Data::FormatMoneyAmount(data.stars, "XTR");
      },
      [](const UnsupportedMedia &data) { Unexpected("Unsupported message."); },
      [](v::null_t) {});
  return result;
}

bool HtmlWriter::Wrap::forwardedNeedsWrap(const Data::Message &message,
                                          const MessageInfo *previous) const {
...
QByteArray HtmlWriter::Wrap::composeStart() {
  ...
  result.append(pushTag("meta", {{"charset", "utf-8"}, {"empty", ""}}));
  result.append(pushTag("title", {{"inline", ""}}));
  result.append("Exported Data");
  result.append(popTag());
  result.append(_context.pushTag(
      "meta", {{"name", "viewport"},
               {"content", "width=device-width, initial-scale=1.0"},
               {"empty", ""}}));
  result.append(_context.pushTag("link", {{"href", _base + "css/style.css"},
                                          {"rel", "stylesheet"},
                                          {"empty", ""}}));
  ...
  result.append(popTag());
  result.append(pushTag("body", {{"onload", "CheckLocation();"}}));
  result.append(pushDiv("page_wrap"));
  return result;
}

HtmlWriter::Wrap::~Wrap() { (void)close(); }

HtmlWriter::HtmlWriter() = default;

Result HtmlWriter::start(const Settings &settings,
                         const Environment &environment, Stats *stats) {
...
Result HtmlWriter::writeDelayedPersonal(const QString &userpicPath) {
  ...
  const auto result =
      writePreparedPersonal(*base::take(_delayedPersonalInfo), userpicPath);
  if (!result) {
    return result;
  }
  if (_userpicsCount) {
...
Result HtmlWriter::writePreparedPersonal(const Data::PersonalInfo &data,
  ...
  userpic.largeLink = userpicPath.isEmpty() ? QString() : userpicsFilePath();
  userpic.imageLink =
      WriteUserpicThumb(_settings.path, userpicPath, userpic, "_info");
...
Result HtmlWriter::writeUserpicsStart(const Data::UserpicsInfo &data) {
  ...
  auto block =
      _userpics->pushHeader("Profile pictures", mainFileRelativePath());
...
Result HtmlWriter::writeUserpicsSlice(const Data::UserpicsSlice &data) {
  ...
  for (const auto &userpic : data.list) {
    ...
    Assert(!file.relativePath.isEmpty() || file.skipReason != SkipReason::None);
    const auto status = [&]() -> Data::Utf8String {
      ...
      case SkipReason::FileType:
        return "(Photo not included. "
               "Change data exporting settings to download.)";
      case SkipReason::None:
        return Data::FormatFileSize(file.size);
      }
    ...
    block.append(_userpics->pushListEntry(
        data, (path.isEmpty() ? QString("Photo unavailable") : path).toUtf8(),
        status,
        (userpic.date > 0 ? Data::FormatDateTime(userpic.date) : QByteArray()),
        path));
  }
...
void HtmlWriter::pushUserpicsSection() {
  pushSection(kUserpicsPriority, "Profile pictures", "photos", _userpicsCount,
              userpicsFilePath());
}
...
Result HtmlWriter::writeStoriesStart(const Data::StoriesInfo &data) {
  ...
  auto block = _stories->pushHeader("Stories archive", mainFileRelativePath());
...
Result HtmlWriter::writeStoriesSlice(const Data::StoriesSlice &data) {
  ...
  for (const auto &story : data.list) {
    ...
    data.imageLink = Data::WriteImageThumb(
        _settings.path, image, kStoryThumbWidth * 2, kStoryThumbHeight * 2);
    const auto info =
        (story.date > 0) ? Data::FormatDateTime(story.date) : QByteArray();
    block.append(_stories->pushStoriesListEntry(
        data, (path.isEmpty() ? QString("Story unavailable") : path).toUtf8(),
        status, info, story.caption, _environment.internalLinksDomain, path));
  }
...
Result HtmlWriter::writeProfileMusicStart(const Data::ProfileMusicInfo &data) {
  ...
  auto block =
      _profileMusic->pushHeader("Profile Music", mainFileRelativePath());
...
Result HtmlWriter::writeProfileMusicSlice(const Data::ProfileMusicSlice &data) {
  ...
  for (const auto &message : data.list) {
    ...
    const auto title = !doc.songTitle.isEmpty() ? doc.songTitle
                       : !doc.name.isEmpty()
                           ? doc.name
    ...
    const auto duration =
        doc.duration > 0 ? Data::FormatDuration(doc.duration) : QByteArray();
    block.append(_profileMusic->pushAudioEntry(
        (path.isEmpty() ? QString("File unavailable") : path).toUtf8(), info,
...
QString HtmlWriter::storiesFilePath() const { return "lists/stories.html"; }

void HtmlWriter::pushStoriesSection() {
  pushSection(kStoriesPriority, "Stories archive", "stories", _storiesCount,
              storiesFilePath());
}
...
void HtmlWriter::pushProfileMusicSection() {
  pushSection(kProfileMusicPriority, "Profile Music", "music",
              _profileMusicCount, profileMusicFilePath());
}
...
Result HtmlWriter::writeSavedContacts(const Data::ContactsList &data) {
  ...
  auto block = file->pushHeader("Contacts", mainFileRelativePath());
  ...
  for (const auto index : Data::SortedContactsIndices(data)) {
    ...
    auto userpic =
        UserpicData{Data::ContactColorIndex(contact), kEntryUserpicSize};
    ...
    block.append(
        file->pushListEntry(userpic, ComposeName(userpic, "Deleted Account"),
  ...
  pushSection(kContactsPriority, "Contacts", "contacts", data.list.size(),
              filename);
  return Result::Success();
}
...
Result HtmlWriter::writeFrequentContacts(const Data::ContactsList &data) {
  ...
  const auto file = fileWithRelativePath(filename);
  auto block = file->pushHeader("Frequent contacts", mainFileRelativePath());
  block.append(file->pushDiv("page_body list_page"));
  block.append(file->pushAbout(_environment.aboutFrequent));
  block.append(file->pushDiv("entry_list"));
  const auto writeList = [&](const std::vector<Data::TopPeer> &peers,
    ...
    for (const auto &top : peers) {
      ...
      auto userpic =
          UserpicData{Data::PeerColorIndex(top.peer.id()), kEntryUserpicSize};
      userpic.firstName = name;
      userpic.lastName = lastName;
      block.append(file->pushListEntry(
          userpic, ComposeName(userpic, "Deleted Account"),
          "Rating: " + Data::NumberToString(top.rating), category));
    }
  };
  ...
  pushSection(kFrequentContactsPriority, "Frequent contacts", "frequent", size,
              filename);
  return Result::Success();
}
...
Result HtmlWriter::writeSessions(const Data::SessionsList &data) {
  ...
  const auto file = fileWithRelativePath(filename);
  auto block = file->pushHeader("Sessions", mainFileRelativePath());
  block.append(file->pushDiv("page_body list_page"));
  ...
  for (const auto &session : data.list) {
    block.append(file->pushSessionListEntry(
        ...
        ((session.applicationName.isEmpty() ? Data::Utf8String("Unknown")
                                            : session.applicationName) +
         ' ' + session.applicationVersion),
        (session.deviceModel + ", " + session.platform + ' ' +
  ...
  pushSection(kSessionsPriority, "Sessions", "sessions", data.list.size(),
              filename);
  return Result::Success();
}
...
Result HtmlWriter::writeWebSessions(const Data::SessionsList &data) {
  ...
  const auto file = fileWithRelativePath(filename);
  auto block = file->pushHeader("Web sessions", mainFileRelativePath());
  block.append(file->pushDiv("page_body list_page"));
  ...
  for (const auto &session : data.webList) {
    block.append(file->pushSessionListEntry(
        ...
        (session.domain.isEmpty() ? Data::Utf8String("Unknown")
                                  : session.domain),
        session.platform + ", " + session.browser,
        {session.ip + " \xE2\x80\x93 " + session.region,
         "Last active: " + Data::FormatDateTime(session.lastActive),
         "Created: " + Data::FormatDateTime(session.created)},
        (session.botUsername.isEmpty() ? QByteArray()
                                       : ('@' + session.botUsername))));
  }
  if (const auto result = file->writeBlock(block); !result) {
    return result;
  ...
  pushSection(kWebSessionsPriority, "Web sessions", "web", data.webList.size(),
              filename);
  return Result::Success();
}
...
Result HtmlWriter::writeOtherData(const Data::File &data) {
  ...
  pushSection(kOtherPriority, "Other data", "other", 1, data.relativePath);
  return Result::Success();
}
...
Result HtmlWriter::writeDialogsStart(const Data::DialogsInfo &data) {
  ...
  auto block = _chats->pushHeader("Chats", mainFileRelativePath());
  block.append(_chats->pushDiv("page_body list_page"));
  if (const auto result = _chats->writeBlock(block); !result) {
    return result;
  }

  pushSection(kChatsPriority, "Chats", "chats",
              data.chats.size() + data.left.size(), "lists/chats.html");
...
Result HtmlWriter::writeDialogSlice(const Data::MessagesSlice &data) {
  ...
  auto oldIndex =
      (_messagesCount > 0) ? ((_messagesCount - 1) / kMessagesInFile) : 0;
  ...
  for (const auto &message : data.list) {
    ...
    if (oldIndex != newIndex) {
      if (const auto result = _chat->writeBlock(block); !result) {
        ...
        _lastMessageIdsPerFile.push_back(saved ? saved->id
                                               : _lastMessageInfo->id);
        block = QByteArray();
        _lastMessageInfo = nullptr;
        previous = nullptr;
    ...
    if (DisplayDate(date, previous ? previous->date : 0)) {
      block.append(_chat->pushServiceMessage(
          --_dateMessageId, _dialog, _settings.path, FormatDateText(date)));
    }
    const auto &[info, content] = _chat->pushMessage(
        message, previous, _dialog, _settings.path, data.peers,
        _environment.internalLinksDomain, messageLinkWrapper);
...
Result HtmlWriter::writeEmptySinglePeer() {
  ...
  return _chat->writeBlock(_chat->pushServiceMessage(
      --_dateMessageId, _dialog, _settings.path, "No exported messages"));
}
...
Result HtmlWriter::writeDialogEnd() {
  ...
  const auto NameString = [](const Data::DialogInfo &dialog) -> QByteArray {
  ...
  const auto LastNameString = [](const Data::DialogInfo &dialog) -> QByteArray {
  ...
  return _chats->writeBlock(_chats->pushListEntry(
      userpic, ComposeName(userpic, DeletedString(_dialog.type)),
...
Result HtmlWriter::validateDialogsMode(bool isLeftChannel) {
  const auto mode = isLeftChannel ? DialogsMode::Left : DialogsMode::Chats;
  ...
  _dialogsMode = mode;
  auto block = _chats->pushAbout(isLeftChannel ? _environment.aboutLeftChats
...
Result HtmlWriter::writeDialogOpening(int index) {
  const auto name = (_dialog.name.isEmpty() && _dialog.lastName.isEmpty())
                        ? QByteArray("Deleted Account")
                        : (_dialog.name + ' ' + _dialog.lastName);
  auto block = _chat->pushHeader(
      name, _settings.onlySinglePeer() ? QString() : _dialogsRelativePath);
  ...
  if (index > 0) {
    ...
    block.append(_chat->pushTag("a", {{"class", "pagination block_link"},
...
void HtmlWriter::pushSection(int priority, const QByteArray &label,
                             const QByteArray &type, int count,
                             const QString &path) {
  _savedSections.push_back({priority, label, type, count, path});
}
...
Result HtmlWriter::writeSections() {
  ...
  if (_savedSections.empty()) {
    ...
    auto block = _summary->pushDiv(_summaryNeedDivider ? "sections with_divider"
                                                       : "sections");
  ...
  auto block = QByteArray();
  ranges::sort(_savedSections, std::less<>(),
               [](const SavedSection &data) { return data.priority; });
  for (const auto &section : base::take(_savedSections)) {
    block.append(_summary->pushSection(section.label, section.type,
                                       section.count,
...
QByteArray HtmlWriter::wrapMessageLink(int messageId, QByteArray text) {
  const auto it =
      ranges::find_if(_lastMessageIdsPerFile, [&](int maxMessageId) {
        return messageId <= maxMessageId;
      });
  if (it == end(_lastMessageIdsPerFile)) {
    return "<a href=\"#go_to_message" + Data::NumberToString(messageId) +
           "\" onclick=\"return GoToMessage(" +
           Data::NumberToString(messageId) + ")\">" + text + "</a>";
  } else {
    const auto index = it - begin(_lastMessageIdsPerFile);
    return "<a href=\"" + messagesFile(index).toUtf8() + "#go_to_message" +
           Data::NumberToString(messageId) + "\">" + text + "</a>";
  }
}
...
Result HtmlWriter::switchToNextChatFile(int index) {
  ...
  auto next = _chat->pushTag(
      "a", {{"class", "pagination block_link"}, {"href", nextPath.toUtf8()}});
...
Result HtmlWriter::finish() {
  ...
  block.append(
      _summary->pushAbout(_environment.aboutTelegram, _summaryNeedDivider));
  if (const auto result = _summary->writeBlock(block); !result) {
    return result;
  }
...
Result HtmlWriter::copyFile(const QString &source,
                            const QString &relativePath) const {
  return File::Copy(source, pathWithRelativePath(relativePath), _stats);
}

QString HtmlWriter::mainFilePath() {
  return pathWithRelativePath(
      _settings.onlySinglePeer() ? messagesFile(0) : mainFileRelativePath());
}
...
QString HtmlWriter::messagesFile(int index) const {
  return "messages" + (index > 0 ? QString::number(index + 1) : QString()) +
         ".html";
}

std::unique_ptr<HtmlWriter::Wrap>
HtmlWriter::fileWithRelativePath(const QString &path) const {
  return std::make_unique<Wrap>(pathWithRelativePath(path), _settings.path,
                                _stats);
}
