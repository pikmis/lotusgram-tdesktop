...
QString NormalizePath(const Settings &settings) {
  ...
  const auto base =
      QString(settings.onlySinglePeer() ? "ChatExport_%1" : "DataExport_%1")
...
std::unique_ptr<AbstractWriter> CreateWriter(Format format) {
  switch (format) {
  case Format::Html:
    return std::make_unique<HtmlWriter>();
  case Format::Json:
    return std::make_unique<JsonWriter>();
  case Format::HtmlAndJson:
    return std::make_unique<HtmlAndJsonWriter>();
  }
...
Stats AbstractWriter::produceTestExample(const QString &path,
  ...
  settings.path =
      (folder.endsWith('/') ? folder : (folder + '/')) + "ExportExample/";
  settings.types = Settings::Type::AllMask;
  settings.fullChats =
      Settings::Type::AllMask &
  ...
  const auto check = [](Result result) { Assert(result.isSuccess()); };
  ...
  const auto date = [&] { return time(nullptr) - 86400 + counter(); };
  const auto prevdate = [&] { return date() - 86400; };
  ...
  const auto generatePhoto = [&] {
    ...
    result.image.file.relativePath =
        "files/photo_" + QString::number(++index) + ".jpg";
    return result;
  };
  ...
  dialogBot.relativePath = "chats/chat_" + QString::number(counter()) + '/';
  ...
  dialogChat.relativePath = "chats/chat_" + QString::number(counter()) + '/';
