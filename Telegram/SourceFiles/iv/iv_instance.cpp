...
public:
  Shown(not_null<Delegate *> delegate, not_null<Main::Session *> session,
        not_null<Data *> data, QString hash);

  [[nodiscard]] bool showing(not_null<Main::Session *> session,
  ...
  [[nodiscard]] rpl::lifetime &lifetime() { return _lifetime; }
...
private:
  ...
  [[nodiscard]] ::Data::FileOrigin
  fileOrigin(not_null<WebPageData *> page) const;
  ...
  void processPartInFile(FileStream &file, Media::Streaming::LoadedPart &&part);
  bool finishRequestWithPart(PartRequest &request,
  ...
  readFile(const std::shared_ptr<::Data::DocumentMedia> &media);
  void requestDone(Webview::DataRequest request, QByteArray bytes,
                   std::string mime, int64 offset = 0, int64 total = 0);
...
Shown::Shown(not_null<Delegate *> delegate, not_null<Main::Session *> session,
             not_null<Data *> data, QString hash)
    : _delegate(delegate), _session(session) {
  prepare(data, hash);
}
    ...
    if (!channel->isLoaded() && !channel->username().isEmpty()) {
      channel->session()
          ...
          .request(MTPcontacts_ResolveUsername(
              MTP_flags(0), MTP_string(channel->username()), MTP_string()))
...
ShareBoxResult Shown::shareBox(ShareBoxDescriptor &&descriptor) {
  ...
  public:
    ...
    not_null<QWidget *> toastParent() const override { return _parent.data(); }
    bool valid() const override { return _lookup() != nullptr; }
    operator bool() const override { return valid(); }
  ...
  const auto layer = Ui::CreateChild<Ui::LayerStackWidget>(
      wrap.get(), [=] { return std::make_shared<Show>(weak.get(), lookup); });
  state->stack = layer;
  ...
  layer->move(0, 0);
  wrap->sizeValue() |
      rpl::on_next([=](QSize size) { layer->resize(size); }, layer->lifetime());
  layer->hideFinishEvents() | rpl::filter([=] {
    return !!lookup(); // Last hide finish is sent from destructor.
  }) | rpl::on_next([=] { state->destroyRequests.fire({}); }, wrap->lifetime());
  ...
  const auto focus = crl::guard(layer, [=] {
    ...
    if (!handle) {
      ...
      *waiting =
          base::qt_signal_producer(qApp, &QGuiApplication::focusWindowChanged) |
...
void Shown::createController() {
  ...
  _controller =
      std::make_unique<Controller>(_delegate, std::move(showShareBox));
...
void Shown::showWindowed(Prepared result) {
  ...
  _controller->show(_session->local().resolveStorageIdOther(),
                    std::move(result), base::duplicate(_inChannelValues));
}
...
void Shown::streamPhoto(QStringView idWithPageId,
                        Webview::DataRequest request) {
  ...
  const auto check = [=] {
    ...
    requestDone(request, media->imageBytes(PhotoSize::Large), "image/jpeg");
    return true;
  };
  if (!check()) {
    photo->session().downloaderTaskFinished() | rpl::filter(check) |
        rpl::take(1) | rpl::start(_controller->lifetime());
  }
}

void Shown::streamFile(QStringView idWithPageId, Webview::DataRequest request) {
  using namespace Data;
  ...
  if (!loader) {
    if (document->size >= Storage::kMaxFileInMemory) {
      ...
      if (const auto content = readFile(media); !content.isEmpty()) {
        requestDone(std::move(request), content,
...
void Shown::streamFile(FileStream &file, Webview::DataRequest request) {
  ...
  const auto till = (request.limit > 0)
                        ...
                        : size;
  const auto parts =
      std::min(int((till + kPart - 1) / kPart) - from, kMaxLoadParts);
  // auto base = IvBaseCacheKey(document);

  const auto length = std::min((from + parts) * kPart, size) - from * kPart;
...
QByteArray
Shown::readFile(const std::shared_ptr<::Data::DocumentMedia> &media) {
  return Lottie::ReadContent(media->bytes(), media->owner()->filepath());
}

void Shown::processPartInFile(FileStream &file,
                              Media::Streaming::LoadedPart &&part) {
  for (auto i = begin(file.requests); i != end(file.requests);) {
    if (finishRequestWithPart(*i, part)) {
      ...
      requestDone(std::move(done.request), done.data, file.mime, done.offset,
                  file.document->size);
    } else {
      ++i;
    }
...
bool Shown::finishRequestWithPart(PartRequest &request,
                                  const Media::Streaming::LoadedPart &part) {
  ...
  memcpy(request.data.data() + index * kPart, part.bytes.constData(), copy);
...
void Shown::streamMap(QString params, Webview::DataRequest request) {
  ...
  const auto prepared = ImageWithLocation{
      .location = ImageLocation({location}, location.width, location.height)};
  auto &preview = _maps.emplace(params, MapPreview()).first->second;
  preview.file = std::make_unique<CloudFile>();

  UpdateCloudFile(*preview.file, prepared, _session->data().cache(),
                  kImageCacheTag,
  ...
  LoadCloudFile(_session, *preview.file, FileOrigin(), LoadFromCloudOrLocal,
                autoLoading, kImageCacheTag, finalCheck, done,
                [=](bool) { done("failed..."); });
}
...
void Shown::requestDone(Webview::DataRequest request, QByteArray bytes,
                        std::string mime, int64 offset, int64 total) {
  if (bytes.isEmpty() && mime.empty()) {
    requestFail(std::move(request));
    return;
  }
  crl::on_main([done = std::move(request.done), data = std::move(bytes),
                mime = std::move(mime), offset, total] {
...
void Shown::requestFail(Webview::DataRequest request) {
  crl::on_main([done = std::move(request.done)] { done({}); });
}

bool Shown::showing(not_null<Main::Session *> session,
                    not_null<Data *> data) const {
...
bool Shown::active() const { return _controller && _controller->active(); }

void Shown::moveTo(not_null<Data *> data, QString hash) { prepare(data, hash); }
...
bool TonSite::active() const { return _controller && _controller->active(); }

void TonSite::moveTo(QString uri) { _controller->showTonSite({}, uri); }
...
void Instance::show(not_null<Window::SessionController *> controller,
                    not_null<Data *> data, QString hash) {
  show(controller->uiShow(), data, hash);
}

void Instance::show(std::shared_ptr<Main::SessionShow> show,
                    not_null<Data *> data, QString hash) {
  this->show(&show->session(), data, hash);
}

void Instance::show(not_null<Main::Session *> session, not_null<Data *> data,
                    QString hash) {
  if (Platform::IsMac()) {
  ...
  const auto guard = gsl::finally([&] { requestFull(session, data->id()); });
      ...
      rpl::on_next(
          [=](Controller::Event event) {
            ...
            case Type::OpenMedia:
              if (const auto window = Core::App().activeWindow()) {
                ...
                const auto controller =
                    (current && &current->session() == _shownSession) ? current
                                                                      : nullptr;
                ...
                if (event.context.startsWith("-photo")) {
                  ...
                  if (!photo->isNull()) {
                    window->openInMediaView({controller, photo, item,
                  ...
                  if (!video->isNull()) {
                    window->openInMediaView({controller, video, item,
                                             topicRootId, monoforumPeerId});
                  }
                }
              }
            ...
            case Type::OpenLink: {
              ...
              session->api()
                  .request(MTPmessages_GetWebPage(MTP_string(url),
                  ...
                  .fail([=] { UrlClickHandler::Open(event.url); })
                  .send();
            } break;
            case Type::Report:
  ...
  session->changes().peerUpdates(::Data::PeerUpdate::Flag::ChannelAmIn) |
      rpl::on_next(
...
void Instance::openWithIvPreferred(
    not_null<Window::SessionController *> controller, QString uri,
    QVariant context) {
  auto my = context.value<ClickHandlerContext>();
  my.sessionWindow = controller;
  openWithIvPreferred(&controller->session(), uri, QVariant::fromValue(my));
}

void Instance::openWithIvPreferred(not_null<Main::Session *> session,
                                   QString uri, QVariant context) {
...
void Instance::showTonSite(const QString &uri, QVariant context) {
...
void Instance::requestFull(not_null<Main::Session *> session,
                           const QString &id) {
  ...
  session->api()
      .request(MTPmessages_GetWebPage(MTP_string(id), MTP_int(requested.hash)))
      .done([=](const MTPmessages_WebPage &result) {
...
WebPageData *Instance::processReceivedPage(not_null<Main::Session *> session,
                                           const QString &url,
  ...
  mtp.match(
      [&](const MTPDwebPageNotModified &data) {
      ...
      [&](const auto &) { requested.page = owner->processWebpage(mtp).get(); });
